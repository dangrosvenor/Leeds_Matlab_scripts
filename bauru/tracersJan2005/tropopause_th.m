function [t,h]=tropopause_th(t_interp,alt_interp,dth_thresh,hposdtdz,hmin)
% [t,h]=tropopause_th(t_interp,alt_interp,dth_thresh,hposdtdz,hmin)

%hposdtdz=1200;

dtdz=diff(t_interp)./diff(alt_interp);
posdt=find(dtdz>dth_thresh & alt_interp(2:end)>hmin);
goflag=1;
ipos=1;
while goflag==1
    ihpos=posdt(ipos);
