...
function [adiabatic_lwc,adjusted_lwc,adjusted_slwc,adtemp,i_status1,i_status2] ...
    = slwc_revb ( cb_pa,cb_k,gt_pa,gt_k,ct_k )
...
%        cb_k  = temperature of this point (note:this is not actually used except for some checking) (K)
%        ct_k  = cloud top temperature (K) (note:this only used for determining likely cloud type for setting i_status flags)
...
%
  if(tg >= 0.|ctt < -20.) i_status1=0; end
%
  tk=t0+273.15;    %cloud base temperature
  e=vapor(t0);  %vapour pressure
