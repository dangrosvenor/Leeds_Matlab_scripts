...
function ztot=Radar_new(Grid,TwoD,ih1,ih2,RHO,ihoriz)
%ihoriz is a flag to say want to compute the reflectivity of a horiz slice
...
%for it=1:4
 for it=[1 2 3 4]   
	...
	if strmatch(lab,'Rain');
       ...
       Z(it).z=paul_zr(cx(it),nxo,lam,alx(it),dx);  %note no 0.19 factor for rain
       ...
       Z(it).z=0.19*paul_zr(cx(it),nxo,lam,alx(it),dx);       
	end
