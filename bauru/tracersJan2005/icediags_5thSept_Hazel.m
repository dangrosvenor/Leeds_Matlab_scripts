...
% 
    for idg=1:length(dgcol(j).d)
        icediagsALL(jc).i(:,jj,idg)=TimeAvDan(idir).DGAV(:,dgcol(j).d(idg)); %saved as icediagsALL_a-b from Nov 2005
    end
    
     for idg=1:length(dgcol2(j).d)
%          switch microcase
%          case 'old'
             icediag(jc).i(:,jj,idg)=TimeAvDan(idir).DGAV(:,dgcol2(j).d(idg));
%          case 'oldnew'
