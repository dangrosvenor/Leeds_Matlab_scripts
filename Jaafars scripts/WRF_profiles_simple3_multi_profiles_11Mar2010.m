...
switch graph
    ...
    case 1  %profiles at different positions in WRF domain
        ...
        location_lab(1).l = 'Forecast'; % Site name to be plotted in the graph as legend (A for site 1)
        location_lab(2).l = 'B';
        location_lab(3).l = 'C';
        location_lab(4).l = 'D';
        %    location_lab(5).l = 'I';
...
% These are the profiles for second WRF output file and third and so on
        wrf_run_names(1).name = 'Reanalysis'; %Run A
...
%             var='wind speed';  %wind speed magnitude sqrt(U^2+V^2)
        ...
        if iRASS_plot==1  %if you want to plot both profile (WRF and RASS) you chose 1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            time=0; %set the time in hours required for first profile (RASS)
            idat=idat+1;
            xdat(idat).x=data_structure(time+1).data(idata_col,:)*factor;
            ydat(idat).y=data_structure(time+1).data(1,:); %
            labs(idat).l='Obs: 13 Feb 00:00LT'; % this is for first profile label and for the second profile see line 895
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %if doing a multi-run plot then set the times here for each run (set to one for analysis)
        time_array=[6 16 1];    % [2] means plot index= 2 (index 2 = 01 hour GMT and index=3 mean 02 hr) to find out
        ...
        %lat, lon coordinates of locations to plot
        LAT_vals = [-65]; % Antarctica - for Dan to check script with his WRF output
        LON_vals = [-65]; %
                ...
                switch var
                    ...
                    case 'RH'
                        ...
                        if is_met_em(1)
                            ...
                            qv = nca(irun).nc{'QVAPOR'}(time,:,ilat(iloc),ilon(iloc));
                            T = WRFUserARW(nca(irun).nc,'tc',time,ilat(iloc),ilon(iloc)) + 273.15;
                            P = WRFUserARW(nca(irun).nc,'p',time,ilat(iloc),ilon(iloc)) *100;
                            qsat = satvappress(T,'goff','liq',P,1)/f;
...
%                                cd(dire2);
                                u=WRFUserARW(nca(irun).nc,'u',time,ilat(iloc),ilon(iloc));
                                v=WRFUserARW(nca(irun).nc,'v',time,ilat(iloc),ilon(iloc));
                            else
                                u=WRFUserARW(nca(irun).nc,'u',time,ilat(iloc),ilon(iloc));
                                v=WRFUserARW(nca(irun).nc,'v',time,ilat(iloc),ilon(iloc));
...
%                                cd(dire2);
                        ...
                        DX=nca(irun).nc.DX(1)/1000;
                        DY=nca(irun).nc.DY(1)/1000;
                        
                        xdat(idat).x=wind_dir_compass_from_uv_wrf(u,v,lat2d,lon2d,ilat(iloc),ilon(iloc),DX,DY);
                ...
                labs(idat).l=['WRF :', location_lab(idat-i_previous).l ' ' hour ' UTC'];%  %simple label more suitable for papers
                 % or you can type it as follows if you need full title: labs(idat).l=['WRF :', location_lab(idat-i_previous).l ' ' day ' ' month_text ' ' hour ':' mins];
