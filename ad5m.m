...
figure;
pcs=[1000 800 700 600 500 400 300 200]; %mb
ths=273:10:353;
thsfine=260:0.1:600;
ts=30:-10:-70;
...
dlogydx=(log(thcs(1,siz))/log(10) - log(thcs(1,1))/log(10)) / ( T(siz)-T(1) );

%thi=0;
%thi= atan( log(thcs(1,siz)) / T(siz) ) - atan(log(thcs(1,1)) / T(1) );


[xx,yy]=rotatedan(T,th,atan(th.*dlogydx)); 
h=plot(xx,(log10(yy)),'r'); %pseudo-adiabat


[x,y]=rotatedan(T,thcs(i,:),atan(thcs(i,:).*dlogydx));
%r(i,:)=( (T.^2)+(log(thcs(i,:)).^2) ).^0.5;
%theta(i,:)=atan( log(thcs(i,:))./T );
h=plot( x,(log10(y)) ); %plot isobars


%rotate(h,[0 0 1],a*180/pi);    
h=text(x(fix(siz*19/20)),log10(y(fix(siz*19/20))),num2str(pcs(i)));
%rotate(h,[0 0 1],a*180/pi);
...
size(ths);
for i=1:ans(2)
   [x,y]=rotatedan(T,ths(i),atan(ths(i).*dlogydx));
   h=plot( x,(log10(y)) );
   %rotate(h,[0 0 1],a*180/pi);
   h=text(x(fix(siz*8/20)),log10(y(fix(siz*8/20))),num2str(ths(i)),'rotation',-19);
   %rotate(h,[0 0 1],a*180/pi);
end
...
size(ts);
for i=1:ans(2)
   [x,y]=rotatedan(273.13+ts(i),thsfine,atan(thsfine.*dlogydx));
   h=plot( x,(log10(y)) ); %plot isotherms
   %rotate(h,[0 0 1],a*10*180/pi);
   h=text(x(fix(siz*3/20)),log10(y(fix(siz*3/20))),num2str(ts(i)));
   %rotate(h,[0 0 1],a*180/pi);
end
...
[x,y]=rotatedan(273.13+Tdat,THdat,atan(THdat.*dlogydx));
h=plot( x,(log10(y)),'kx'); %plot sounding
