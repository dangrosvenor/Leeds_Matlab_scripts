...
clear x a2 map c rhs time date dir vals zeros np aa
...
for ifile=1141:arad
    ii=ii+1
    if strcmp('MAX',d(ifile).name(1:3))==1
    %if strcmp(d(ifile).name,'ppi_bru_240204_2100h.gif')==1
        
        day(ii)=24;
        month(ii)=2;
        year(ii)=2004;
        hrs(ii)=str2num(d(ifile).name(13:14));
        %hrs=mod(hrs+3,24); %add 3 hrs to convert to UTC
        mins(ii)=str2num(d(ifile).name(15:16));
