function [x,timser_meancomp,icomp,ilat2,ilon2]=timeseries_plots_SMOS_func(mon_thresh,Nwindow,Soil_Moisture,vars_in)
...
ntime = size(Soil_Moisture,3);
%figure
timser_comp=NaN*ones([ntime*2+1 length(ilat2)]);
timser=NaN*ones([ntime length(ilat2)]);
icomp=0;
for ireg=1:length(ilat2)
    timser(:,ireg)=squeeze(Soil_Moisture(ilat2(ireg),ilon2(ireg),:));
    x = [1:ntime];
    y = timser(:,ireg);
    inan =find(isnan(y)==1);
    x(inan)=[]; y(inan)=[];
