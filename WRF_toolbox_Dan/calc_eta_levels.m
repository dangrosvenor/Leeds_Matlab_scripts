...
res_top=1700;
dz=50; %high resoltuion of the lower part of domain (below res_top)
...
res_aim = interp1(zeta(2:end),diff(zeta),z_abv_top); %resolution at this height - interpolate from the existing z values
...
z_res = [0 z_grid zeta(imin+1:end)]; %note the height corresponding to ptop is missing, which is why e_vert=length(z_res)+1 in namelist.input

file_eta = 'c:/documents and settings/dan/my documents/wrf/eta_levels_BL_50m';
