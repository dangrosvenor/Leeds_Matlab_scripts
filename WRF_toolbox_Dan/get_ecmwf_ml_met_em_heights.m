function [heights]=get_ecmwf_ml_met_em_heights(nc,ilat,ilon)
%function [heights]=get_ecmwf_ml_met_em_heights(nc,ilat,ilon)
%get the heights for ECMWF met_em data when model levels were used (no GHT)

iloc=1;

  
                if prod(size(nc{'GHT'})) > 0
                    heights = nc{'GHT'}(1,:,ilat(iloc),ilon(iloc));
                else
