...
for iz=80:90   %size(potemp.var,1)-1
            
            i380=find(potemp.var(iz,:,:)>380); %indices of all potemps above 380K
            %i380=i380(1); %first index
            %i380=83;
            ...
            if length(i380)>0
                mass_air = pressure.var(iz,i380).*28.97e-3/8.3144./(temp.var(iz,i380)+273.15) ...
                    .* (z_wrf2.var(iz+1,i380) - z_wrf2.var(iz,i380)) *dx*dy;
                
                qt=qtotal(iz,i380);
                qv=qvap(iz,i380);
                
                val=mean(qt(:))+3*std(qt(:));   %threshold for selecting overshoot points - 3 std devs away from mean
                val2=mean(qt(:))+1*std(qt(:));   %threshold for selecting icy overshoot points - 1 std devs away from mean
                
                ibig=find(qt>val);                
                iice=find(  qt>val | ( qt-qv > 0.025/f & qt>val2 )  );
                
                ifind=find(f*qt<prctile(f*qt(:),40));
                val3=mean(f*qt(ifind))+5*std(f*qt(ifind));
