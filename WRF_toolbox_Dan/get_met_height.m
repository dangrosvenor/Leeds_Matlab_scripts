function [output]=get_met_height(nc,ilat,ilon,iloc,one_height,pres_end)
%finds height profile of the location for index ilat(iloc),ilon(iloc)
%function [output]=get_met_height(nc,time,ilat,ilon,iloc,one_height)

if prod(size(nc{'GHT'})) > 0
    if one_height>0
        h = nc{'GHT'}(time,:,ilat(iloc),ilon(iloc));
        P = nc{'PRES'}(time,:,ilat(iloc),ilon(iloc));
        output = interp1(h,P,one_height);
    else
        output = nc{'GHT'}(1,:,ilat(iloc),ilon(iloc));
    end

else
    %
    pres=nc{'PRES'}(:); pres=pres(:,ilat(iloc),ilon(iloc))'; 
    ...
    if pres_end>0
            PSPAN=[pres2(1) pres_end]; %pressure range for integration
    else
    ...
    if one_height>0
        output=interp1(hp,P,one_height);
    else
        output = interp1(P,hp,pres);
    end
