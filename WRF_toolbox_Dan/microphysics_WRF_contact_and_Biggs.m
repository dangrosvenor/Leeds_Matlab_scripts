function [final_contact_rate, rate_bigg] = microphysics_WRF_contact_and_Biggs(tc,P,QC3D)
...
%%% immersion nucleation (of cloud droplets to form ice) - Bigg's #/L/s
rate_bigg=CONS40*exp(log(CDIST1)+log(gamma(PGAM+4))-3*log(LAMC)).*exp(AIMM*(273.15-T3D)) .*RHO/1000;
