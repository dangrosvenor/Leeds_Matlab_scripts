...
/*        mexPrintf("\n DPG nDim = %d, vnDimIn[0]=%d, vnDimIn[1]=%d  \n",nDim, vnDimIn[0], vnDimIn[1]); */
    ...
    */
    ...
    int iDim = 0;
     ...
     if (!((int )vnDimIn & (int )vnDimOut & (int )vnEdge & (int )vCoord & (int )vpvEdge & (int )vnAdd))   
/*     DPG - Above was causing a bug - removed vnDimIn check as suggested on Matlab File Exchange - actually, this did not help with the issue - I think the key was to compile using the Olympus gcc compiler. */
/*    if (!((int )vnDimOut & (int )vnEdge & (int )vCoord & (int )vpvEdge & (int )vnAdd)) */
    {
/*        mexPrintf("\n vnDimIn == %d : vnDimOut == %d : vnEdge == %d: vCoord=%d : vpvEdge=%d : vnAdd=%d \n",(int )vnDimIn, (int )vnDimOut, (int )vnEdge , (int )vCoord , (int )vpvEdge , (int )vnAdd)); */
        mexPrintf("\n vnDimIn == %d : vnDimOut == %d : vnEdge == %d :vCoord=%d : vpvEdge=%d : vnAdd=%d \n",(int )vnDimIn, (int )vnDimOut,(int )vnEdge,(int )vCoord,(int )vpvEdge,(int )vnAdd);

        mexErrMsgTxt("Error with ndHistc.c - Memory allocation error (1)");
    }
