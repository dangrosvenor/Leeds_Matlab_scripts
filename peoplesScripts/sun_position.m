...
% this script.
if ~isstruct(t_input)
    tt = datevec(t_input); %for some reason the call to this here is slow
    %is quicker if datevec is run outside of this function and then the
    %arguments passed in - see sun_pos2 and sun_position_Dan
    time.UTC=0;
     time.year = tt(1);
     time.month = tt(2);
     time.day = tt(3);
     ...
     time.min = tt(5);
     time.sec = tt(6);
% [time.year,time.month,time.day,time.hour,time.min,time.sec] = datevec(t_input);
