...
% Mehmet �ZT�RK 2011
% Dan - Returns a cell array lin{i} each of which is a martix of size [3 n]
% These are n points at which the intersection occurs xj=(1,j), yj=(2,j),
% zj=(3,j). Not sure what the different i values are? Multiple regions that
% intersect??
...
% 
% p0=[0 0 0]; v=[0 0 1]; % a plane that parallel to the xy plane and passes the point [0 0 0]
% n=v./norm(v);
% 
...
if nargin==0
    error('�ntersectPlaneSurf: not enough input arguments')
elseif nargin==1
    if size(fv.faces,1)~=3
        error('�ntersectPlaneSurf: surface data must be defined with triangels')
    end
    error('�ntersectPlaneSurf: The plane must be defined with a point p0 and a normal vector of the plane n')
elseif nargin==2
    error('�ntersectPlaneSurf: The plane must be defined with 2 arguemnts')
end
