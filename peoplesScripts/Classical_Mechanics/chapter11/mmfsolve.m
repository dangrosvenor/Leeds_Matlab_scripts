...
% file downloaded from http://www.mathworks.com/matlabcentral/
% modified the double "&&" and "||" to "&" and "|" to run on earlier R12
...
% Dan - example usage for solving 2 equations (in 2 variables):
% [Xf,Fval,ExitFlag,Iters]=MMFSOLVE(@mmfsolve_test_Dan,[1.1 0]')
% I.e. need to make Xo a column vector. Xo(1) and Xo(2) are the solution guesses for the
% two variables.
% mmfsolve_test_Dan returns the values of the two functions for given
% values for the two variables (also as a column vector, size=[2 1])
...
if ischar(varargin{1}) % check for Options=MMFSOLVE('Name','Value',...)
   ...
   if ~isempty(strncmpi(varargin{1},Pnames,length(varargin{1})))
