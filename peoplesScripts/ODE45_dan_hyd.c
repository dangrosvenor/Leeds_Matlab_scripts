...
/* Dan - added these */
/* #define	Tprof_TEMP		tmp_prhs[2]
   #define	Pprof_TEMP		tmp_prhs[3] */
...
mexFunction(int nlhs, mxArray *plhs[],int nrhs, mxArray *prhs[])
{
	...
	mxArray	*tmp_plhs[1],*tmp_prhs[4], *tmp_prhs2[1];  /* Dan - increased dimension of tmp_prhs from 2 to 4 for the extra inputs */
	mxArray	*s_matptr[6];
	mxArray	*old_plhs[2];
	...
	*/
	...
	/*	if (!mxIsNumeric(T0_IN) || mxIsComplex(T0_IN) || 
		!mxIsSparse(T0_IN)  || !mxIsDouble(T0_IN) || nr*nc != 1)
	...
	/*	if (!mxIsNumeric(TF_IN) || mxIsComplex(TF_IN) || 
		!mxIsSparse(TF_IN)  || !mxIsDouble(TF_IN) || nr*nc != 1)
	...
	/*	if (!mxIsNumeric(Y0_IN) || mxIsComplex(Y0_IN) || 
		!mxIsSparse(Y0_IN)  || !mxIsDouble(Y0_IN) || !M)
	...
	Y_OUT = mxCreateDoubleMatrix(mxGetM(Y0_IN),mxGetN(Y0_IN), mxREAL);
	...
	T_TEMP = mxCreateDoubleMatrix(1, 1, mxREAL);
	Y_TEMP = mxCreateDoubleMatrix(mxGetM(Y0_IN),mxGetN(Y0_IN), mxREAL);
	/* Dan - added Tprof_TEMP and Pprof_TEMP here */
	/*        Tprof_TEMP = mxCreateDoubleMatrix(mxGetM(Tprof),mxGetN(Tprof), mxREAL);
		  Pprof_TEMP = mxCreateDoubleMatrix(mxGetM(Pprof),mxGetN(Pprof), mxREAL);  */

	/*        Tprof_TEMP = mxGetPr(Tprof);
		  Pprof_TEMP = mxGetPr(Pprof);  */

	/*	        TP = mxGetPr(Tprof);
	  PP = mxGetPr(Pprof);   */

	/*		  tmp_prhs2[0] = prhs[6];  
	  mexCallMATLAB(1,tmp_plhs,4,tmp_prhs,fcn_name); test call that worked*/
...
/* Dan - add the pointers for the temp and pressure array to tmp_prhs */

	tmp_prhs[2] = prhs[5];
        tmp_prhs[3] = prhs[6];

	/*	TP2 = mxGetPr(tmp_prhs[2]); */

	/*	mexPrintf("\nValues, Tprof_TEMP[0]=%f, Pprof_TEMP[0]=%f\n",Tprof_TEMP[0],Tprof_TEMP[0]);   */
	/*	mexPrintf("\nValues, Tprof_TEMP[0]=%f, Pprof_TEMP[0]=%f\n",TP[0],TP[1]); */
	...
	if (nrhs < 5) {
		...
		/*		if (!mxIsNumeric(TOL_IN) || mxIsComplex(TOL_IN) || 
			!mxIsSparse(TOL_IN)  || !mxIsDouble(TOL_IN) || nr*nc != 1)
	...
	*t = t0;
	...
	while ((*t < tfinal) && ((*t + h) >= *t)) {
		...
		*t = ts;
		...
		/* mexPrintf("\nValues, Running function\n"); */

  /* --- first call to the Matlab function ---- */
                /* Dan - changed from mexCallMATLAB(1,tmp_plhs,2,tmp_prhs,fcn_name); to below to increase no. input args to Matlab func */
		mexCallMATLAB(1,tmp_plhs,4,tmp_prhs,fcn_name);
		...
		*t = ts + h*alpha[4];
		...
		if (delta <= tau) {
			...
			OLD_T_OUT = T_OUT;
			OLD_Y_OUT = Y_OUT;
			T_OUT = mxCreateDoubleMatrix(1,K+1,mxREAL);
			Y_OUT = mxCreateDoubleMatrix(m,K+1,mxREAL);
			...
			if (k) {
				mxDestroyArray(OLD_T_OUT);
				mxDestroyArray(OLD_Y_OUT);
			}
