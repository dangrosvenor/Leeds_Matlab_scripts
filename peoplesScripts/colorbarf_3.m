...
if loc(1)=='v', % Append vertical scale to right of current plot
    ...
    colors =[-8.0000 -6.2794   -4.5589   -2.8383   -1.1177 NaN];
    
    
    lc=length(colors);
    % Create color stripe by drawing the appropriate number of filled rectangles 
    if any(isnan(colors))&dont==1 % if the bottom level is not filled
       for j = 1:lc
    ...
    myYticks = ylimits(1):(ylimits(2)-ylimits(1))/lc:ylimits(2)...
       -((ylimits(2)-ylimits(1))/lc);
    set(gca,'ytick',myYticks);
    
    myStr{1} = ' ';
    for kk = 1:length(colors)
       myStr{kk+1} = num2str(colors(kk));
    end
    myStr{(length(C2)+2)} = ' ';
