function sun = sun_position_Dan(Y,M,day_of_month,H,MI,S,location)
...
%   calculated. 
%       year_vec: year. Valid for [-2000, 6000]
%       month_vec: month [1-12]
%       day_vec: calendar day [1-31]
%       hour_vec: local hour [0-23]
%       min_vec: minute [0-59]
%       sec_vec: second [0-59]
%       utc_vec: offset hour from UTC. Local time = Greenwich time + utc_vec
%   This input can also be passed using the Matlab time format ('dd-mmm-yyyy HH:MM:SS'). 
%   In that case, the time has to be specified as UTC time (utc_vec = 0)
%
...
julian = julian_calculation_Dan(Y,M,day_of_month,H,MI,S);
