...
%d0_start=0.2e3;
d0_start=d(end)-0.2e3;
for h=h0-4/L/n:-4/L/n:-4/L  %solves for each point up the mountain - from h0 back down to 0 and
                    % then below 0 (negative mountain height as can have an asymmetric mountain)
    i=i+1
     sign0=sign(mountain_smith_func(d0_start+0.2e3,h,L,H0) );
     for d0=[d0_start:-0.2e3:dmax]
        if mountain_smith_func(d0,h,L,H0) < 1e-90
            d2(i)=d0;
            ifzero=1;
            break
        else
...
dh = 100*L; %to find speeds along a line that is 100m above the mountain surface
