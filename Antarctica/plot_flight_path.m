%Use read_ant06_2 to read the flight19 data (Antarctic Peninsula).
...
eval_str=['ntot = findheight_nearest(time_flt' flight_no ',time_inds);';];
eval(eval_str);
...
iuse_another_domain_for_flight_path_details=0;

if iuse_another_domain_for_flight_path_details==1
    XR = 1440; %position of a reference point on outer domain (Rothera)
    YR = 1463;
    
    XR2 = (ilon(1)-1)*DX; %position of Rothera on the high-res domain
    YR2 = (ilat(1)-1)*DY;
