...
fcb_stretch = 1.7; %2.2
size_fac = 0.99; %0.85;
dY = 0.05;  %0.05 %distance to move the subplot upwards relative to papersize
dX = 0.0;
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
         t1 = title(textwrap({'CAM5'},max_tit_wrap));
...
%        set(gca,'ylim',[225 300]);
         t2 = title(textwrap({'AM3'},max_tit_wrap));
...
% ---- optional settings ------        
 %       xlabel('');
  %      set(gca,'ylim',[0.014 0.019]);
         title(textwrap({'AM3CLUBBv1'},max_tit_wrap));
    end
...
%        ylabel('$\theta_{2 km}$','interpreter','latex');
%        set(gca,'ylim',[225 300]);
         title(textwrap({'AM3CLUBBv2'},max_tit_wrap));
% ------------------------------         
  end
