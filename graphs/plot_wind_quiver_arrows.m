...
%u=ones(size(u))*0; %useful for testing the wind scale arrows
%v=ones(size(v))*10;
...
%     y_grid = ( max(y_draw)-min(y_draw) ) / ( length(y_draw) - 1 );
    ...
    x_draw = x(yinds,xinds);
    y_draw = y(yinds,xinds);
    %size of the grid boxes for x and y
%     x_grid = diff(x,1,2);
%     x_grid(:,end+1) = x_grid(:,end);
%     y_grid = diff(y,1,1);
%     y_grid(end+1,:) = y_grid(end,:);    

end

 x_grid = ( maxALL(x_draw)-minALL(x_draw) ) / ( length(xinds) - 1 );
 y_grid = ( maxALL(y_draw)-minALL(y_draw) ) / ( length(yinds) - 1 );
...
if nargin>=7 %if want to scale vectors - e.g. in order to make arrows proportional to wind speed on different plots
    ...
    if iplot_single==1
        ...
        off_pos_y=dY_single/25;
        off_pos_y=dY_single/6;
        ...
        single_arrow_fine_tune_y_factor = 0.33;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        v_single(length(yinds)+ioffy,length(xinds)+ioffx) = scale_speed(2)*width_factor;
        ...
        try
            ...
            text(x_single(end)-0.0*x_grid,y_single(length(yinds)+ioffy)-y_grid*0.95,[num2str(scale_speed(1)) ' m/s'] );
            text(x_single(end),y_single(length(yinds)+ioffy+1)+y_grid*0.75,[num2str(scale_speed(2)) ' m/s'] );
        end
