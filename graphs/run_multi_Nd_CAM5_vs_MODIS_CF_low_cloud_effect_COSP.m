...
size_fac = 0.95;
dY = 0.05;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = 0.0;
...
dY_cb = [0.5 0.0 -0.07 0.0 0.0];  %0.75;  %0.11; %distance to move the colorbars upwards
dX_cb = [-0.02 -0.02 -0.02 -0.02 -0.02];

dY_spacing = 0.02; %vertical spacing to add between plots, negative values add space
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
%        title(textwrap({'CAMCLUBBv2 vs MODIS all CF'},max_tit_wrap));
         title(textwrap({'Low cloud only, all CF, no COSP screening'},max_tit_wrap));         
    end
...
%         title(textwrap({'CAMCLUBBv2 vs MODIS CF>0.8'},max_tit_wrap));
         title(textwrap({'Low cloud only, all CF, COSP re required'},max_tit_wrap));         
     end
...
%    delete(Hcb(3)); %delete the first colorbar - replace with a vertical one
%    Hcb(3) = colorbar('vert','peer',hsub_new(1));
% --------------------------------------------------

    xlabel(Hcb(3),'% N_{d} bias for CAM5');
%    delete(Hcb(3));
% ------------------------------
...
%        set(gca,'ylim',[225 300]);
%         title('MOD35, r_e<14 \mum all CF');
        title(textwrap({'Low cloud only, all CF, COSP re required, COSP CF>0.1'},max_tit_wrap));      
% ------------------------------         
    end
...
if Ntot>=5
    dY = dY + dY_spacing;  %add on dY_spacing before odd numbered plots
     dY_cb(5) = dY_cb(5) + dY_spacing;  %add on dY_spacing before odd numbered plots
