...
fcb_stretch = 2.0; %1.7; %2.2
fcb_stretch_vert = 0.35; %Amount to stretch in the vertical

%Note for size_fac doesn't seem to do anything until get below a certain
%size...
size_fac = 0.27; %0.99; %0.85;
dY = 0.4; %-0.02;  %0.05 %distance to move the subplot upwards relative to papersize
dX = 0.0;
...
dY_cb = 0.5;  %0.11; %distance to move the colorbars upwards
dX_cb = 0.0; %0.05

dY_spacing = 0.02; %vertical spacing to add between plots, negative values add space

dY_day_lab=0.1; %Upaward spacing between day/night labels and a,b,c labels
%hcol_text = text(tit_pos(1)-0.04,tit_pos(2)+dY_labs+dY_day_lab,str);
...
if stretch_cb==1
    ...
    pos_new = [pos_cb{1}(1)+dX_cb pos_cb{1}(2)+dY_cb pos_sub{1}(3)*fcb_stretch pos_cb{1}(4)*fcb_stretch_vert];
else
...
%delete(Hcb(1)); fprintf(1,'\n ***Deleting colorbar 1 - check don''t need it! ***')
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
         t1 = title(textwrap({''},max_tit_wrap));
...
%        set(gca,'ylim',[225 300]);
         t2 = title(textwrap({''},max_tit_wrap));
...
if Ntot>=3
    ...
    %axis position is [left bottom width height]
    if stretch_cb==1
        pos_new = [pos_cb{3}(1)+dX_cb pos_cb{3}(2)+dY_cb pos_sub{3}(3)*fcb_stretch pos_cb{3}(4)*fcb_stretch_vert];
    else
...
%        set(gca,'ylim',[225 300]);
         title(textwrap({'CAMCLUBBv1'},max_tit_wrap));
% ------------------------------         
    end
...
if Ntot>=5
    ...
    stretch_cb=1;
    if stretch_cb==1
        pos_new = [pos_cb{5}(1)+dX_cb pos_cb{5}(2)+dY_cb pos_sub{5}(3)*fcb_stretch pos_cb{5}(4)*fcb_stretch_vert];
    else
...
%        ylabel('$\theta_{2 km}$','interpreter','latex');
%        set(gca,'ylim',[225 300]);
         title(textwrap({'CAMCLUBBv2'},max_tit_wrap));
% ------------------------------         
 end
