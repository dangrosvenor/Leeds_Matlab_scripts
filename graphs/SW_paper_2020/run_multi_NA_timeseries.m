...
posit=[0 scrsz(4) scrsz(3)*0.5 scrsz(4)];
...
for ifig=1:length(fig_load)
    ...
    [hsub_new(ifig),Hcb(ifig),pos_sub{ifig},pos_cb{ifig},Hleg{ifig}] = load_fig_copy_to_subplotv2(hf,fig_dir,fig_load{ifig},ifig,fontsize,Nsub,Msub,clims{ifig});
 ...
 % though there was no colorbar
    ...
    if strcmp(fig_tit{ifig},'ORIGINAL') ~= 1
...
savename_out = saveas_ps_fig_emf(gcf,savename,'',0,0,0);
