...
fcb_stretch = 1.7; %2.2
size_fac = 0.7; %0.85;
dY = 0.12;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = -0.05;
...
dX_cb = -0.05
dY_spacing = -0.02; %vertical spacing to add between plots
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
         title(textwrap({'MOD35, absolute difference, w/ screening'},max_tit_wrap));
    end
...
%        set(gca,'ylim',[225 300]);
         title(textwrap({'MOD35, percentage difference, w/ screening'},max_tit_wrap));
         delete(Hcb(2)); %delete the first colorbar - replace with a vertical one
         hc_new(2) = colorbar('vert','peer',hsub_new(2));
         %xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
         ylabel(hc_new(2),'%','fontsize',fontsize-4);
     end
...
%        set(gca,'ylim',[225 300]);
         title('MOD35, absolute difference, w/ screening');
% ------------------------------         
    end
