function hh = quiverDan2(varargin)
%QUIVER Quiver plot.
...
%x(isnan(u)&isnan(v))=NaN;
%y(isnan(u)&isnan(v))=NaN;
...
h1 = plot(uu(:),vv(:),[col ls],'parent',cax);
...
uu = ( uu - xmin ) / ( xmax - xmin );
vv = ( vv - ymin ) / ( ymax - ymin );
%%%%

if min(size(x))==1, n=sqrt(numel(x)); m=n; else [m,n]=size(x); end

theta = pi/2 - atan ( abs( (uu(2,:) - uu(1,:)) ./ (vv(2,:) - vv(1,:)) ) );

d=sqrt ( (uu(2,:)-uu(1,:)).^2 + (vv(2,:)-vv(1,:)).^2 )/3; 
%make length of each arrowhead arm one third length of arrow stem

%alpha = 3*pi/4 - atan( abs( (uu(2,:) - uu(1,:)) ./ (vv(2,:) - vv(1,:)) ) );
alpha = 3*pi/4 - atan( ( (uu(2,:) - uu(1,:)) ./ (vv(2,:) - vv(1,:)) ) );
...
dx = -d.*fsign.*cos(alpha);
dy = -d.*fsign.*sin(alpha);

dx2 = -d.*fsign.*cos(pi/2 - alpha);
dy2 = d.*fsign.*sin(pi/2 - alpha);


hu = [uu(2,:)+dx2; uu(2,:); uu(2,:)+dx;repmat(NaN,size(u));];
hv = [vv(2,:)+dy2; vv(2,:); vv(2,:)+dy;repmat(NaN,size(v));]; 
%make [4 n] array of coords for left side of arrowhead, middle and rightside plus NaNs
%NaNs at end are needed to stop it connecting from last point to the next
...
%hu = [uu(2,:); uu(2,:)];
%hv = [vv(2,:); vv(2,:) - (vv(2,:)-vv(1,:))/4];

  hold(cax,'on')

hu = xmin + hu*(xmax - xmin);  %to scale back from a square grid to actual x,y coords, Dan
hv = ymin + hv*(ymax - ymin);
    
%  axis square

%  h2 = plot('v6',hu(:),hv(:),[col ls],'parent',cax);
    h2 = plot(hu(:),hv(:),[col ls],'parent',cax);
else
  h2 = [];
end
...
%  h3 = plot('v6',hu(:),hv(:),[col ms],'parent',cax);
    h3 = plot(hu(:),hv(:),[col ms],'parent',cax);
