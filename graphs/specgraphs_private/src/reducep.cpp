...
static char rcsid[] = "$Revision: 1.8.4.5 $";
...
void reduce_init(Model *M0, double *v, double *f, mwSize nvin, mwSize nfin, int verbose)
{
    mwSize i;
    mwSize initialVertCount;
    mwSize initialEdgeCount;
    mwSize initialFaceCount;
...
void reduce_execute( Model *M0, int face_target, int verbose, double *vOut, double *fOut, mwSize *nv, mwSize *nf, mwSize nvout, mwSize nfout)
{
    mwSize i, j;
    mwSize uniqVerts = 0;
    mwSize numOrigFaces,verboseIncrement;
...
void
reduce( int target, double *v, double *f, mwSize nvin, mwSize nfin, int verbose, mwSize nvout, mwSize nfout, double *vOut, double *fOut, mwSize *nv, mwSize *nf)
{
    Model M0;
		 ...
		 )
{
    double   *v, *f;
    double   *vOut, *fOut;
    mwSize ndims, nvin, nfin, nvout, nfout, nv, nf;
    int	   target, verbose;
