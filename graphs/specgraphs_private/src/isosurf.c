...
static char rcsid[] = "$Revision: 1.10.4.1 $";
...
mwSize XYDIM;
double DATA[8];
mwSize currentTriangle, currentVertex;
 ...
 */
void createTriangle( mwSize index0, mwSize index1, mwSize index2 )
{
 ...
 */
mwSize findNextEdge(mwSize thisedge, int* nextEdgeValid)
{
    mwSize j, edge, nextedge = 0;
    *nextEdgeValid = 0;
 ...
 */
void generatePolygons(mwSize x, mwSize y, mwSize z, int noshare)
{	
    mwSize i,j;
    mwSize nextedge, nedges, edges[12];
      ...
      if (curedges[i].valid)
	{
	    ...
	    while(1)
	      {
              int nextEdgeValid = 0;
		  nextedge = findNextEdge(edges[nedges-1],&nextEdgeValid);
		  if (!nextEdgeValid || nextedge==edges[0] || nedges==12)
		    {
			for (j=0; j<nedges; j++)
 ...
 */
int calcEdgeValues(double *data,mwSize x,mwSize y,mwSize z,mwSize xdim,mwSize ydim,mwSize zdim,double isovalue)
{
    register double *tmp;
    double val, d;
 ...
 */
void calcEdgeColorValues(double *colors,mwSize x,mwSize y,mwSize z,mwSize xdim,mwSize ydim,mwSize zdim)
{
    register double *tmp;
    int i, v0, v1;
 ...
 */
int isoSurface(double *data, double *colors, mwSize xSize, mwSize ySize, mwSize zSize, double isovalue, int noshare, int verbose)
{

  register mwSize x,y,z;
