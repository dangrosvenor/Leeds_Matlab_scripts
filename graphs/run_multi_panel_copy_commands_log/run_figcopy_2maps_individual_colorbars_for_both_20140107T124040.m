...
fcb_stretch = 1.7; %2.2
size_fac = 0.8;  %0.27;  %0.78; %0.85;

dY = 0.13; %0.105;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = -0.05;
even_only_dX = 0; %flag to tell it to only move the even numbered plot by dX
...
%set(Hcb(ifig),'yaxislocation','right');
...
delete(Hcb(1)); %delete the right colorbar
%axis position is [left bottom width height]
...
%    pos_new = [0.89 0.1 0.03 0.8];
    pos_new = [0.35 0.45 0.35 0.03];
end

% set(Hcb(1),'position',pos_new);
% set(Hcb(1),'fontsize',fontsize-4);
hc_new(1) = colorbar('vert','peer',hsub_new(1))
%xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
ylabel(hc_new(1),'Frequency','fontsize',fontsize-4);
% % ------------------------------
...
%          set(Hcb(2),'fontsize',fontsize-4);
%          xlabel(Hcb(2),'Frequency','fontsize',fontsize-4);
         
           hc_new(2) = colorbar('vert','peer',hsub_new(1))
           %xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
           xlabel(hc_new(2),'Frequency','fontsize',fontsize-4);
    end
