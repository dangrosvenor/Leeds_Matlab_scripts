...
size_fac = 0.75;
dY = 0.1;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = 0.0;
extra_for_even = 1; %flag to tell it to only move the odd numbered plot by dX
dX_even = -0.1; %move the even plots by this much extra

dY_cb = [0.5 0.0 -0.05 0.0 0.0];  %0.75;  %0.11; %distance to move the colorbars upwards
dX_cb = [-0.02 -0.02 -0.02 -0.02 -0.02];

dY_spacing = 0.0; %vertical spacing to add between plots, negative values add space
...
%        title(textwrap({'CAMCLUBBv2 vs MODIS all CF'},max_tit_wrap));
         title(textwrap({'CAM5 N_d'},max_tit_wrap));         
    end
...
%         title(textwrap({'CAMCLUBBv2 vs MODIS CF>0.8'},max_tit_wrap));
         title(textwrap({'CAMCLUBBv2 N_d'},max_tit_wrap));         
     end
...
%         title('MOD35, r_e<14 \mum all CF');
         title('CAM5 vs MODIS bias (all CF)');         
% ------------------------------         
    end
...
%         title('MOD06, r_e<14 \mum all CF');
         title('CAMCLUBBv2 vs MODIS bias (all CF)');
    end
