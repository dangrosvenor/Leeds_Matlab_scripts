...
Nsub = 3;
Msub = 1;  %No. of panels Nsub = no. vert (no. rows), Msub = no. horiz (no. columns)

%for colorbar labels search for 
%   title(textwrap({
...
fcb_stretch = 0.4; %1.7 %2.2
size_fac = 0.25; %0.99; %0.85;
size_fac = 1.0; %0.8
dY = 0.0;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = -0.3;
...
%Positive moves the images upwards
dY_cb = [0.0 0.0 -0.12 0.0 0.0];  %0.75;  %0.11; %distance to move the colorbars upwards
dX_cb = [0.0 0.0 -0.07 0.0]; %negative moves left
dY_spacing = [0.06 0.03 0.03 -0.1 -0.1 -0.1 -0.1]; %vertical spacing to add between plots, negative values add space
...
%pos_cb is not applied to the colorbar in load_fig_copy_to_subplotv2 and
%so is not its actual position. Haven't changed in there as might mess up
%other plotting scripts. So just get the actual position here.
pos_cb{ifig} = get(Hcb(ifig),'Position');
...
if resizing==1
    ...
    for ifig2=1:length(fig_load)
        ...
        set(hsub_new(ifig2),'outerposition',[outpos(1) outpos(2) outpos(3)*size_fac outpos(4)*size_fac]);
    end
...
dY(1) = dY(1) + dY_spacing(Ntot);
dY_cb(1) = dY_cb(1) + dY_spacing(Ntot);


if stretch_cb==1
    %the one below stretches the colorbar
    pos_new = [pos_cb{1}(1)+dX_cb(1) pos_cb{1}(2)+dY_cb(1) pos_sub{1}(3)*fcb_stretch pos_cb{1}(4)];
else
    pos_new = [pos_cb{1}(1)+dX_cb(1) pos_cb{1}(2)+dY_cb(1) pos_sub{1}(3) pos_cb{1}(4)];
end
 set(Hcb(1),'position',pos_new);
 xlabel(Hcb(1),'cm^{-3}','fontsize',fontsize-2);
% % ------------------------------
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
         title(textwrap({'Daytime'},max_tit_wrap));         
    end


if Ntot>=2
%     row_no = ceil(Ntot/Nsub);
% 
%     if row_no>0
%         dY = dY + dY_spacing;  %add on dY_spacing before odd numbered plots
%         dY_cb(3) = dY_cb(3) + dY_spacing;  %add on dY_spacing before odd numbered plots
%     end

    dY = dY + dY_spacing(Ntot);
    dY_cb(2) = dY_cb(2) + dY_spacing(Ntot);
...
%        set(gca,'ylim',[225 300]);
         title(textwrap({'Nighttime'},max_tit_wrap));         
     end
         
     
     if stretch_cb==1
         %the one below stretches the colorbar
         pos_new = [pos_cb{2}(1)+dX_cb(2) pos_cb{2}(2)+dY_cb(2) pos_sub{2}(3)*fcb_stretch pos_cb{2}(4)];
     else
         pos_new = [pos_cb{2}(1)+dX_cb(2) pos_cb{2}(2)+dY_cb(2) pos_sub{2}(3) pos_cb{2}(4)];
     end
     set(Hcb(2),'position',pos_new);
     xlabel(Hcb(2),'cm^{-3}','fontsize',fontsize-2);
     % % ------------------------

     delete(Hcb(2)); %delete the second colorbar

end
    

if Ntot>=3
%    col_no = rem(Ntot,Msub);
%    if col_no==1; col_no=3; end
%     row_no = ceil(Ntot/Nsub);
%     
%  if row_no>0   
%     dY = dY + dY_spacing;  %add on dY_spacing before odd numbered plots
...
%  end
    ...
    dY = dY + dY_spacing(Ntot);
    dY_cb(3) = dY_cb(3) + dY_spacing(Ntot);
    
    % ---- optional settings ------
%    delete(Hcb(4)); %delete the right colorbar
    %axis position is [left bottom width height]
    if stretch_cb==1
        pos_new = [pos_cb{3}(1)+dX_cb(3) pos_cb{3}(2)+dY_cb(3) pos_sub{3}(3)*fcb_stretch pos_cb{3}(4)];
    else
        pos_new = [pos_cb{3}(1)+dX_cb(3) pos_cb{3}(2)+dY_cb(3) pos_sub{3}(3) pos_cb{3}(4)];
    end
    set(Hcb(3),'position',pos_new);
    xlabel(Hcb(3),'g m^{-2}','fontsize',fontsize-2);
%    delete(Hcb(3));
% ------------------------------
...
%        set(gca,'ylim',[225 300]);
         title(textwrap({'Nightime minus daytime'},max_tit_wrap));       
% ------------------------------         
    end
...
%     end
    ...
    dY = dY + dY_spacing(Ntot);
    dY_cb(4) = dY_cb(4) + dY_spacing(Ntot);
    
    stretch_cb=1;
    if stretch_cb==1
        pos_new = [pos_cb{4}(1)+dX_cb(4) pos_cb{4}(2)+dY_cb(4) pos_sub{4}(3)*fcb_stretch pos_cb{4}(4)];
    else
        pos_new = [pos_cb{4}(1)+dX_cb(4) pos_cb{4}(2)+dY_cb(4) pos_sub{4}(3) pos_cb{4}(4)];
    end
    set(Hcb(4),'position',pos_new);
%    xlabel(Hcb(4),'%','fontsize',fontsize-2);
...
% ---- optional settings ------        
 %       xlabel('');
  %      set(gca,'ylim',[0.014 0.019]);
         title(textwrap({'CF 0.8-1.0'},max_tit_wrap));  
    end
...
%     end
    ...
    dY = dY + dY_spacing(Ntot);
    dY_cb(5) = dY_cb(5) + dY_spacing(Ntot);  %add on dY_spacing before odd numbered plots
    ...
    stretch_cb=1;
    if stretch_cb==1
        pos_new = [pos_cb{5}(1)+dX_cb(5) pos_cb{5}(2)+dY_cb(5) pos_sub{5}(3)*fcb_stretch pos_cb{5}(4)];
    else
        pos_new = [pos_cb{5}(1)+dX_cb(5) pos_cb{5}(2)+dY_cb(5) pos_sub{5}(3) pos_cb{5}(4)];
    end
    set(Hcb(5),'position',pos_new);
    xlabel(Hcb(5),'%','fontsize',fontsize-2);
% ------------------------------
...
%     end
    ...
    dY = dY + dY_spacing(Ntot);
    dY_cb(6) = dY_cb(6) + dY_spacing(Ntot);
...
%    xlabel(Hcb(ifig2),'%','fontsize',fontsize-2);
% ------------------------------
