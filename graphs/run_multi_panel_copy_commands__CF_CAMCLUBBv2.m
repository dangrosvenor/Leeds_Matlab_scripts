...
fcb_stretch = 1.7; %2.2
size_fac = 0.85;  %0.78; %0.85;
dY = 0.105;  %0.09; %0.02; %distance to move the subplot upwards relative to papersize
dX = -0.10;
even_only_dX = 1; %flag to tell it to only move the odd numbered plot by dX
dY_cb = 0.125;  %0.11; %distance to move the colorbars upwards
dY_spacing = -0.025; %vertical spacing to add between plots
...
if stretch_cb==1
 ...
 xlabel(Hcb(1),'Low Cloud Fraction','fontsize',fontsize-4);
% % ------------------------------
...
if Ntot>=3
    ...
    xlabel(Hcb(3),'Mid Cloud Fraction','fontsize',fontsize-4);
% ------------------------------
...
if Ntot>=5
    ...
    xlabel(Hcb(5),'High Cloud Fraction','fontsize',fontsize-4);
% ------------------------------
