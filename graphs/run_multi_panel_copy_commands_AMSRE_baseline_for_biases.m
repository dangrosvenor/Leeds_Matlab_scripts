...
dY_cb = 0.18;  %0.11; %distance to move the colorbars upwards
dX_cb = -0.05
dY_spacing = 0.0; %vertical spacing to add between plots
...
delete(Hcb(1)); %delete the first colorbar - replace with a vertical one
hc_new(1) = colorbar('vert','peer',hsub_new(1));
%xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
ylabel(hc_new(1),'g m^{-2}','fontsize',fontsize-4);
...
%        xlabel('');
%        set(gca,'ylim',[3 7]);
         title(textwrap({'All r_e all CF'},max_tit_wrap));
    end
...
%        set(gca,'ylim',[225 300]);
         title(textwrap({'r_e<14 \mum all CF'},max_tit_wrap));
         delete(Hcb(2)); %delete the original colorbar - replace with a vertical one
         hc_new(2) = colorbar('vert','peer',hsub_new(2));
         %xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
         ylabel(hc_new(2),'g m^{-2}','fontsize',fontsize-4);
     end
...
%        set(gca,'ylim',[225 300]);
         delete(Hcb(3)); %delete the original colorbar - replace with a vertical one
         hc_new(3) = colorbar('vert','peer',hsub_new(3));
         %xlabel(Hcb(1),'Frequency','fontsize',fontsize-4);
         ylabel(hc_new(3),'g m^{-2}','fontsize',fontsize-4);
         title(textwrap({'r_e<14 \mum CF>0.8'},max_tit_wrap));
% ------------------------------         
    end
...
% ---- optional settings ------        
 %       xlabel('');
  %      set(gca,'ylim',[0.014 0.019]);
         title('MOD06, absolute difference, w/ screening');
    end
