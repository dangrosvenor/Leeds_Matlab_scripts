...
onexlabel=0; %flag to make it so that only the bottom plot has the xlabel on it
...
maxcovOvr = 250.000000;
    ...
    case 'liq'
		...
		mincovOvr = dlog(0.000000,dlogmin);
		maxcovOvr = dlog(160.000000,dlogmin);
        
%     mincovOvr = 0.000000;
% maxcovOvr = 160.000000;
...
%                pdat(1).p=fact*squeeze(sum(TwoDDan(jc).Q(izmin:izmax,:,[2:6]),3));  %total condensate 
       ...
       ix=findheight(GridDan(idir).Y1,-500e3);
       ix2=findheight(GridDan(idir).Y1,500e3);
...
% end
        ...
        case 'lowtracer'                
            pdat(1).p=squeeze(sum(TwoDDan(idir).Q(izmin:izmax,xinds,[10]),3)); %low tracer
        case 'totwater'                
...
%            pdat(1).p=1000*squeeze(sum(TwoDDan(idir).Q(izmin:izmax,:,[2:6]),3)); %tot condensate
        ...
        case 'temppert'
            ...
            T=squeeze(sum(TwoDDan(idir).TH1,3))+tref; %potemp
            pref=repmat(GridDan(idir).PREFN,[1 length(GridDan(idir).Y1)]); %ref p
            P=TwoDDan(idir).PP; %actual p
...
%                     end
    ...
    if onexlabel==0 | (i==nplots2d & onexlabel==1)
        xlabel(xlabelstr);
    end
    ...
    if normcbar==0
      ...
      if (logflag==1 | dlogflag==1 ) & nocbar==0
...
%    text(timesTH(i).t(1)-(timesTH(i).t(end)-timesTH(i).t(1))*0.12,((zz(1).z(end)-zz(1).z(1))*0.08+zz(1).z(end))/1000,['Time = ' timlab ' UTC'],'fontsize',18);
     text(0,0,['Time = ' timlab ' UTC'],'units','centimeters','position',[-2.5 11.5],'fontsize',18);
...
%    text(timesTH(i).t(1)-0.5,ylims(1)*0.988,[direcDan(idir).dir],'fontsize',12);    
    text(timesTH(i).t(1)-dist2,ylims(1)-dist,[runName(idir).nam],'fontsize',12);    
