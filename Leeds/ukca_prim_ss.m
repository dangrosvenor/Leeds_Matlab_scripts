function [numflux_accum, numflux_coarse, massflux_accum, massflux_coarse,drlo,drmid,drhi] = ukca_prim_ss(u)

%u=[1:50];
%u=[4.1 5 10];
%for iu=1:length(u)
iu=1;
[aer_mas_primss,aer_num_primss,aer_numflux_primss,aer_massflux_primss,drlo,drmid,drhi] = ukca_prim_ss_FUNC(u);
numflux_accum=aer_numflux_primss(:,:,:,3);
numflux_coarse=aer_numflux_primss(:,:,:,4);
massflux_accum=aer_massflux_primss(:,:,:,3);
massflux_coarse=aer_massflux_primss(:,:,:,4);
%end
...
''; %add breakpoint here to probe variables

function [aer_mas_primss,aer_num_primss,aer_numflux_primss,aer_massflux_primss,drlo,drmid,drhi] = ukca_prim_ss_FUNC(u_scalar_10m)
...
aer_massflux_primss = zeros(size(aer_mas_primss));
...
m86flux = zeros(size(u_scalar_10m));
s98flux = zeros(size(u_scalar_10m));
dfdr = zeros(size(u_scalar_10m));
combflux = zeros(size(u_scalar_10m));
flux = zeros(size(u_scalar_10m));
boxflux = zeros(size(u_scalar_10m));
deln_mflux = zeros(size(u_scalar_10m));
...
dum=3.0.*mm(cp_cl)/(avc.*rhocomp(cp_cl).*4.0.*pi); %conversion factor for mass.^(1/3) to radius
for jv=1:nbins
    ...
    if (drhi(jv) > cutoff)       % if bin size is > cutoff for flux
        ...
        if (modemt > 0)
            ...
            if (drlo(jv) > cutoff)
                ...
                flux(:,:) = dfdr(:,:).*(drhi(jv) - cutoff).*1.0e6; %.*1e6 to convert to microns
            end
...
%DPG - deln is not acutally used, though, so don't need the air mass...
            ...
            aer_num_primss(:,:,1,modemt) =                              ...
             ...
             % DPG .. sum up each emitted boxflux number into mode in #/m2/s
            aer_numflux_primss(:,:,1,modemt) =                              ...
                aer_numflux_primss(:,:,1,modemt) + boxflux(:,:);

             % DPG .. sum up each emitted boxflux number multiplied by bin mass to give kg/m2/s
            aer_massflux_primss(:,:,1,modemt) =                              ...
                aer_massflux_primss(:,:,1,modemt) + boxflux(:,:).*mbmid(jv).*mm(cp_cl)/avc;
                %DPG - Seems that mdmid etc.is the total number of
                %molecules in each aerosol, rather than the mass - so need
                %to multiply by molecular weight / Avogadro
