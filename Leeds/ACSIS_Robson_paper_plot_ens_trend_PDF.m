...
%% Define the bins
dat_range = maxALL(dat)-minALL(dat);
Nbins = 3;

%Ybins_DRIVER = [-0.01 10.^[log10(20):0.15:log10(2500)]]; ichoose_Ybins=1;   
%Ybins_DRIVER = [1:3:1000];
Ybins_DRIVER = [minALL(dat):dat_range/Nbins:maxALL(dat)]; ichoose_Ybins=1;  

%% Define the data somewhere
%Y_driver = [1:1000]; %the data
Y_driver = dat; %the data
ylabelstr='SW TOA trend (W m^{-2} yr{-1})';
