...
[dat_annual_box_ukesm_actual,dat_annual_box_ukesm_actual_ens_std,trend_dat_box_actual] = ACSIS_Robson_paper_CALC_timeseries(dat_ukesm_save,dat_PI,LAT_val,LON_val,iscreen_land,gcm_area_UM,...
    yr_start_trend_box,yr_end_trend_box,p_conf,ibox);
...
switch var_ukesm
     ...
     case 'DEEPC_fluxes';
        ...
        dat_tmp = SW_TOA_deepc;
        if iscreen_land==1
        	land_mask_MODIS=load('/home/disk/eos1/d.grosvenor/amsre_land_mask.mat');
            lmask_MODIS = flipdim(land_mask_MODIS.amsre_land_mask,1);
            lmask_MODIS = lmask_MODIS + 1; %Make it ones where have ocean    
            lmask_deepc = griddata(land_mask_MODIS.gcm_Plat2D_AMSRE,land_mask_MODIS.gcm_Plon2D_AMSRE,lmask_MODIS,gcm_Plat2D_deepc,gcm_Plon2D_deepc,'nearest');
            for it=1:size(SW_TOA_deepc,1)
...
leg_str{ileg}=['Calculated x ' num2str(f_sw_calc)]; ileg=ileg+1;
set(h1,'linewidth',4);
