...
Y_driver = squeeze(Y_driver);
clear opts; opts.screen_type_optional = land_ocean;
[Y_driver,lmask_out] = SW_Robson_obs_land_mask_FUNC2(Y_driver,gcm_Plat2D_UM,gcm_Plon2D_UM,opts);
%  switch land_ocean
%         case 'land+ocean'
%             % Don't need to do anything
%         case 'ocean only'            
%             Y_driver = Y_driver_screened;
%         case 'land only'
%             land_only_mask = ones(size(lmask_out));
%             land_only_mask(lmask_out==1)=NaN;
%             Y_driver= Y_driver.*land_only_mask;
%             
%  end
    

%% Define the time data (or just use indices)
daynum_timeseries3=[1:size(squeeze(Y_driver),3)];
