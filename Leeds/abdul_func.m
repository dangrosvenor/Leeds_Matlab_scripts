...
dens_aero = density;
clear density %so can use the function called density
...
% aerosol properties
nmodes = length(sig);   % number of modes
vantHoff = [3.0,3.0]; %no. ions of salt that dissociate
massMole = [132.0e-3,132.0e-3]; %Molecular weight of ammonium sulphate (NH4)*2 SO4
...
%          es=SatVapPress(T,'buck2','liq'); %saturation vapour pressure of liquid in Pa
        ...
        bigG=1.0/(rhow*(Rv*T/(es*Dv)+Lv*(Lv/(Rv*T)-1)/(ka*T))); %Eqn. A4 of Ghan (2011) noting that Rv=R/Mw
        ...
        for i=1:nmodes
            ...
            eta=(w(n)*alpha/bigG).^1.5/(2.0*pi*rhow*gamma*N_aero(i)); %Eqn. (35) of Ghan (2011)
               %Ghan states:- eta = 2*(alpha*w/bigG).^(3/2) / (gamma_star * N)
               % gamma_star = 4*pi*rhoW*gamma/rho_air
               % which amounts to the same thing, but UM assumes rho_air=1
               
            %Think this is missing from UM implementation - might be
            %realted to per m3 vs per kg? I think it is since in Ghan I
            %assume N is in per m3 and in the UM in per kg
            %A density of 1.1 vs 1 doesn't
            %make too much difference anyway - but might do higher up?
%            rhoA = p./Rd./T;
%            eta = eta * rhoA;
