...
[dat_annual_box_ukesm,dat_annual_box_ukesm_ens_std,trend_dat_box,...
    trend_dat_box_ens,trend_ens_dat_box, dat_annual_box_ukesm_ens,...
    me_t_PI,N_t_PI,std_t_PI] ...
    = ACSIS_Robson_paper_CALC_timeseries(dat_ukesm,dat_PI,LAT_val,LON_val,iscreen_land,gcm_area_UM,...
...
%% Calculate the OBS MODIS trend for the boxed region
for it_trend=1:1 %length(yr_start_trend_box_obs)
    yr_start=years_obs_modis(1); yr_end=2014; %yr_end=years_obs(end);  %yr_end_trend_box_obs(it_trend);
    ...
    [trend_dat_box_obs_modis{ibox,it_trend}] = ACSIS_Robson_paper_compute_trends_FUNC(yr_start, yr_end, ...
            it_trend, years_obs_modis, obs_annual_box_modis , p_conf);
...
if iplot_individual_ens==1
    ...
    trend_str=['trend=(' num2str(fscale*trend_dat_box{ibox,1}.coeffs(2),'%.2f') '\pm' num2str(fscale*trend_dat_box{ibox,1}.uncer2,'%.2f') ') ' fscale_str];
end
...
if length(years_obs)>1
    h2=plot(years_obs_modis,obs_annual_box_modis,'r-');
else
    h2=plot(years_obs_modis,obs_annual_box_modis,'rs');
end
trend_str=['trend=(' num2str(fscale*trend_dat_box_obs_modis{ibox,1}.coeffs(2),'%.2f') '\pm' num2str(fscale*trend_dat_box_obs_modis{ibox,1}.uncer_max,'%.2f') ') ' fscale_str];
%leg_str{ileg}=[obs_str_modis ', trend=' num2str(trend_dat_box_obs_modis{ibox,1}.coeffs(2),'%.2e')]; ileg=ileg+1;
leg_str{ileg}=[obs_str_modis ', ' trend_str]; ileg=ileg+1;
...
if iplot_lin==1
   ...
   for it_trend=1:length(yr_start_trend_box)
      ...
      switch tail_test
          ...
          case 2     
              if iplot_individual_ens==1
                  conf_str = num2str(100*(1 - trend_dat_box_ens{ibox,it_trend,ens_plot}.T2),'%1.1f');
              else
   ...
   plot( trend_dat_box_obs_modis{ibox,it_trend}.x , trend_dat_box_obs_modis{ibox,it_trend}.ylin , 'r-','linewidth',3,'HandleVisibility','Off');   
   switch tail_test
       case 1
           conf_str = num2str(100*(1 - trend_dat_box_obs_modis{ibox,it_trend}.T1),'%1.1f');
       case 2
           conf_str = num2str(100*(1 - trend_dat_box_obs_modis{ibox,it_trend}.T2),'%1.1f');
   end
   conf_str_tit = [conf_str_tit '; p_{MODIS}=' conf_str '%'];
...
prc_bias_annual = 100*(model_interp./obs_annual_box_modis - 1);
[stats_regions{ibox}]=calc_stats(1e-10,obs_annual_box_modis,model_interp);
