function [dat_annual_box_ukesm,dat_annual_box_ukesm_ens_std,...
    trend_dat_box,trend_dat_box_ens,trend_ens,dat_annual_box_ukesm_ens,...
    me_t_PI,N_t_PI,std_t_PI,itrend_start,itrend_end,N_dat_annual_box_ukesm,...
    std_dat_annual_box_ukesm,Ntot_dat_annual_box_ukesm,Nfrac_dat_annual_box_ukesm]=ACSIS_Robson_paper_CALC_timeseries2(dat_ukesm,dat_PI,LAT_val,LON_val,land_ocean,...
    yr_start_trend_box,yr_end_trend_box,p_conf,ibox)


dat_annual_box_ukesm=[];
dat_annual_box_ukesm_ens_std=[];
trend_dat_box=[];
trend_dat_box_ens=[];
trend_ens=[];
dat_annual_box_ukesm_ens=[];
me_t_PI=[]; N_t_PI=[]; std_t_PI=[];

do_ens=1;
if ~isfield(dat_ukesm,'dat_annual_ens')
   do_ens=0; 
end
...
%     end
        ...
        %eos10 may be lost - try a different run on eos15
    ...
    switch land_ocean
        ...
        case 'ocean, sea-ice regions only'   
            %Load the mask for seaice regions based on max annual surface albedo for DAMIP runs over all ens members
            % Calculated using ACSIS_Robson_paper_Max_surf_albedo.m            
            max_surf_dir = ['/home/disk/eos1/d.grosvenor/modis_work/ACSIS_Nd_trends/'];
            max_surf_name = [max_surf_dir 'max_surf_albedo_DAMIP_annual'];
...
%    clear dat_annual_box_ukesm dat_annual_box_ukesm_ens dat_annual_box_ukesm_ens dat_annual_box_ukesm_ens_std
    for it=1:size(dat_ukesm.dat_annual,1)
        ...
        if do_ens==1
            dat_mean_ens = squeeze(dat_ukesm.dat_annual_ens(:,it,:,:)) .* squeeze(land_mask_ens);
            if length(size(dat_mean_ens))==2
                dat_mean_ens = reshape(dat_mean_ens,[1 size(dat_mean_ens,1) size(dat_mean_ens,2)]);
            end
        end
        
        dat_tmp = dat_mean(ilat,ilon);
        area_tmp = dat_ukesm.gcm_area_UM(ilat,ilon);                
        
        %weight by area of gridbox
        [dat_annual_box_ukesm(it),N_dat_annual_box_ukesm(it),std_dat_annual_box_ukesm(it)] = meanNoNan(dat_tmp(:),1,'',0,1,area_tmp(:)); 
         % function [me,nnums,stdev]=meanNoNan(dat,ndim,op,isqueeze,iweight,weights)
         Ntot_dat_annual_box_ukesm = length(ilat)*length(ilon); %total no. points in region
         Nfrac_dat_annual_box_ukesm(it) = N_dat_annual_box_ukesm(it)/Ntot_dat_annual_box_ukesm;
        
         if do_ens==1
             area_rep = repmat(dat_ukesm.gcm_area_UM(ilat,ilon),[1 1 size(dat_mean_ens,1)]);
...
%     trend_dat_box{ibox,it_trend}.T2 = T2;
    ...
    [trend_dat_box{ibox,it_trend},itrend_start(it_trend),itrend_end(it_trend)] = ACSIS_Robson_paper_compute_trends_FUNC(yr_start_trend_box, yr_end_trend_box, ...
