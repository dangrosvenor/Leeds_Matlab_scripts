...
SSw = e/esatw(nTa, 'Buck81')*100 - 100;

SSi = e/esati(nTa,'Buck81')*100 - 100;
...
SSwa = ea/esatw(aTa, 'Buck81')*100 - 100;
SSwb = eb/esatw(bTa, 'Buck81')*100 - 100;
SSia = ea/esati(aTa,'Buck81')*100 - 100;
SSib = eb/esati(bTa,'Buck81')*100 - 100;
...
Tz = Tc + z*delT;
ez = ec + z*(ew - ec);
SSwz = ez./esatw(Tz, 'Buck81')*100 - 100;

SSiz = ez./esati(Tz,'Buck81')*100 - 100;
...
%% These are functions that are used to find the boundaries of the lamina
    function flux = Flux(p)
        ...
        flux = 1.5*velocmean*(p-p*z/(3*d2)) + ...
            Gam*delT*(z^2/(4*d3)-z/(2*d));
    end
