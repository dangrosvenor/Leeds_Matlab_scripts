...
for j=(k-1)*jmax+1:min(jmax,nplots);
    ...
    %if j==jmax
        text(max(xxmax)/1000/1.25,-max(zzmax)/1000/10,titlecb);
        %end
...
for j=1:nplots
    axis(hs((k-1)*jmax+j).h,[-maxXX maxXX 0 maxZZ]);
end;
...
exname=strcat('c:\matlabR12\work\',exdir,'dump16.jpg');	
