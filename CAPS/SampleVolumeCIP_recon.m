...
dof = min(dof, max_dof);
eaw=CIPRES*1.0e-3*( arrayWidth + 2*(bin/2 - bin/7.25) ); %reconstructed eaw
%effective sample width in mm (or rather the free sample width 
...
%eaw =CIPRES*1.0e-3*( (arrayWidth-2)-bin);         
eaw =CIPRES*1.0e-3*arrayWidth; %not a function of size for centre-in analysis
