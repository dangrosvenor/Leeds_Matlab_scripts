...
time_turb=ncT{'time'}(:)/1e3/3600; %convert to hours - is the same as the aircraft time anyway
u_turb=ncT{'u'}(:);
v_turb=ncT{'v'}(:);
w_turb=ncT{'w'}(:);
T_turb=ncT{'atn'}(:);
...
w2_turb=w_turb;
w2_turb(inan)=NaN;  %perhaps a good idea to do this as otherwise when the inan indices are just
