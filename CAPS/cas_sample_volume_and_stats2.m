function [sample_volume_CAS,sample_volume_CIP,air_speed_1D,air_speed,CAS_total_number...
    ...
    ,LWC_dist_cas_cutoff,LWC_size_dist,bin_range,LWC_dist_cas_cutoff2,MVD,MVD_cut_off]...
...
%do the mode normalised for bin width - i.e. the mode of dN/dlogD
log_bins = log10(CAS_bins);
dlogD=repmat(diff(log_bins),[1 size(CAS_counts,1)])';
dNdlogD = CAS_counts(:,2:end)./dlogD./sample_volume_CAS(:,2:end);  
...
[nmax,imax] = max( dNdlogD ,[], 2); imax=imax+1;
...
%now calculate the LWC, etc if we assume that the numbers are spread monotonically across each bin (i.e. a cumulative
%graph would be linear across the bin - should this be in log space perhaps?
%calculate D2^4-D1^4/(D2-D1)/4 and replicate over the time dimension 
%(from the intergration of pi*rhow*N/(D2-D1) * integral(over D2-D1) of (D^3) dD
Db4_minus_Da4_1D = ( (1e-4*CAS_bins(2:end)).^4 - (1e-4*CAS_bins(1:end-1)).^4 ) ...
    ./ (1e-4* (CAS_bins(2:end) - CAS_bins(1:end-1)) ) /4; %converting um to cm with the sizes
Db4_minus_Da4 = repmat(Db4_minus_Da4_1D,[1 size(CAS_counts,1)])';

LWC_dist_cas_cutoff2 = 1e6*( sum( pi*Db4_minus_Da4(:,icut_off_lwc-1)/6 * rho_w .* CAS_counts(:,icut_off_lwc) ,2 )  )...
    ./ sample_volume_CAS(1:size_CAS,1) ; %in g/m^3

%rho_w is in g/cm3 so convert LWC to g/cm3
MVDi = Db4_minus_Da4 .^(1/3);  %each bin has MVD (cm) of this
%MVD = 1e4*(1e-6*6*LWC_dist_cas_cutoff2/pi/rho_w).^(1/3); %convert LWC from g/m3 to g/cm3 by *1e6 
%then convert to microns from cm

%do a number weighted mean of MVDi sum(MVDi*Ni)/sum(Ni)
%(ignore the sample volume as it will cancel out)
MVD_cut_off = 1e4*( sum( MVDi(:,icut_off_lwc-1) .* CAS_counts(:,icut_off_lwc) ,2 )  )...
     ./ sum(CAS_counts(:,icut_off_lwc),2) ; %

MVD = 1e4*( sum( MVDi(:,:) .* CAS_counts(:,2:end) ,2 )  )...
     ./ sum(CAS_counts(:,2:end),2) ; %
