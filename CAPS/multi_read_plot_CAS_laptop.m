...
switch comp
    case 'comp'
        flight_dir='Y:\BAS_flights\';
    case 'laptop'
        flight_dir='C:\Documents and Settings\G\My Documents\logbook\Antarctica\Flights and instruments\';
    end
    
%filedir = [flight_dir 'flight99_6thFeb2010\'];
%filedir=[flight_dir 'CAPS_cals_02-02-10\'];
filedir=[flight_dir 'flight104\'];
%filedir=[flight_dir 'caps_cals_2010_2\CAPS_cals_08-02-10\'];
%filedir=[flight_dir 'caps_cals_2010_2\CAPS_cals_10-02-10\'];
%filedir=[flight_dir 'caps_cals_2010_2\CAPS_cals_14-02-10\'];
%filedir=[flight_dir 'caps_cals_2010_2\CAPS_cals_02-02-10\'];
%filedir=[flight_dir 'caps_cals_2010_2\CAPS_tests_31-01-10\]';
%filedir=[flight_dir '29thJune2010_CAS_comparisons\bas_cas_29-06-10\'];
%N.B. files 6,7,8 and 9 correspond to the calibrations of 15,20,30 and 40 microns

%filedir=[flight_dir '29thJuly2010_BAS_chamber_comparisons\BAS CAS\'];

%think that the one below contains all the requried ones
%filedir=[flight_dir '29thJuly2010_BAS_chamber_comparisons\100729_cas_intercomparison_data\cas_probes\bas_cas\'];
...
iselect_cas_file=1; %flag to say that want to select a cas file - will ask to select a file
...
if iselect_cas_file==1
    for ifile=1:length(files)        
        ...
        if length(files(ifile).name)>3
            if findstr(files(ifile).name(end-3:end),'.txt') & length(findstr(files(ifile).name,'_s1.txt'))==0 ...
                    & length(findstr(files(ifile).name,'ncd'))==0
                icount=icount+1;
...
if length(files(ifile).name)>3
    text_file = findstr(files(ifile).name(end-3:end),'.txt');
else
    text_file = 0;
end

    if text_file & length(findstr(files(ifile).name,'_s1.txt'))==0 ...
            & length(findstr(files(ifile).name,'cpc'))==0 & length(findstr(files(ifile).name,'ncd'))==0
