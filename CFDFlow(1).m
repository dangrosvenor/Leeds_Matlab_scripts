...
%x(2,1) = fzero(@fx, r);
x(2,1) = fzero(@fx, r,[],d,p,velocmean,Gam,delT,WidthMean,Q,fas);
disp('Hello9')
% This is the b value (closest to warm wall)
%x(3,1) = fzero(@gx, r);
x(3,1) = fzero(@gx, r,[],d,p,velocmean,Gam,delT,WidthMean,Q,fbs);
...
%% These are functions that are used to find the boundaries of the lamina
    %function flux = Flux(p)
    function flux = Flux(a,d,p,velocmean,Gam,delT)
        z = p^2;
        disp('Hello10')
        d2 = d^2;
        d3 = d^3;
        flux = 1.5*velocmean*(p-p*z/(3*d2)) + Gam*delT*(z^2/(4*d3)-z/(2*d));   %%%
        disp('Hello11')
    end
    ...
    %function F = fx(a)
    function F = fx(a,d,p,velocmean,Gam,delT,WidthMean,Q,fas)
   
        %F = Flux(a) - Flux(-d) - fas*Q/WidthMean;
        F = Flux(a,d,p,velocmean,Gam,delT) - Flux(-d,d,p,velocmean,Gam,delT) - fas*Q/WidthMean;
    end

    %function G = gx(b)
    function G = gx(b,d,p,velocmean,Gam,delT,WidthMean,Q,fbs)
        
        %G = Flux(d) - Flux(b) - fbs*Q/WidthMean;
        G = Flux(d,d,p,velocmean,Gam,delT) - Flux(b,d,p,velocmean,Gam,delT) - fbs*Q/WidthMean;
