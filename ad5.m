...
size(thcs);
siz=ans(2);
thi=atan( ((thcs(1,siz))/(10) - (thcs(1,1))/(10)) / ( T(siz)-T(1) ) );
%thi=0;
%thi= atan( log(thcs(1,siz)) / T(siz) ) - atan(log(thcs(1,1)) / T(1) );
[xx,yy]=rotatedan(T,(th)/(10),thi);
h=plot(xx,(yy),'r'); %pseudo-adiabat


[x,y]=rotatedan(T,(thcs(i,:))/(10),thi);
...
size(ts);
for i=1:ans(2)
   [x,y]=rotatedan(273.13+ts(i),(thsfine)/(10),thi);
   h=plot( x,(y) );
...
[x,y]=rotatedan(273.13+Tdat,(THdat)/(10),thi);
