clear Nnew NnewT Mav Mav2 macc macc2 mmav2
...
if addstartbins==1
    ...
    for j=1:nb
        ...
        fbin=snew30(j+1)-snew30(j);
        fval=snew30(j);
        n=nex(j);
        for i=1:n
            snew=[snew fval+(i-1)*fbin/n];
        end
    end
snew=[snew snew30(j+1:end)];    
end
...
nns=[];   
 ...
 for j=1:length(Sc)
  ...
  if weight==1
      ...
      if count+1<=length(snew)
       ...
       elseif NnewT(count)<x*0.9 & flag>0 & it<itmax %bin width can be made larger
          if count==22
              '';
          end
 ...
 if domass==1
       ...
       mm2=sum( c * nns ./ sss.^2 ); %total mass of bin
       mmav2(count)=mm2/sum(nns); %average mass for each bin
 end
