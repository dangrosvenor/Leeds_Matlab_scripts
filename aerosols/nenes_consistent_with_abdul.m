function [I,spart,smax,Nd,is,Nnew]=nenes_consistent_with_abdul(smax,P,T,W,N,sc,density)
%finds smax as for Nenes and Seinfeld parameterisation
%sc= supersaturation vector in %
%smax in %
%Nd is output in per cc
...
%rhoS=density; %density=1.769e3; %density of salt kg/m^3 %ammonium sulphate=1.77e3
...
% --- end of original values


% Abdul constants
L = 0.2501e7;    % Latent heat of vapourization
...
mu=3; %no. ions of salt that dissociate
Ms=132.0e-3; %molecular weight of salt kg/mol
rhoS=density; %density of salt kg/m^3 %ammonium sulphate=1.77e3
tensW = 0.8e-1;  % Surface tension at solution-air interface

Tc = T - 273.15;
Ps=(100.0*6.1121)*exp((18.678-Tc/(234.5))*Tc/(257.14+Tc));
Dv = 0.226e-4;    % diffusivity of water vapour in air
Ka = 0.243e-1;    % thermal conductivity of air

%end of constants, etc.
...
if imax==1; goA=0; goB=0; Ia=0; Ib=0; end %smax smaller than min sc so abort and return zero integrals
...
for j=ipart-1:imax-2
	...
	sumb(j)= N(j)*A(j)/(sc(j+1)-sc(j)) * log(sc(j+1)./sc(j)) + (N(imax-1)*A(imax-1)/( sc(imax)-sc(imax-1) )) * log(smax/sc(imax-1)); %changed imax to imax-1 for N
end
...
if length(is)>0
	...
	if is>=2
        ...
        Nnew(1:is-1)=0; %deplete all bins up to is
        
	else
        Nd=0;
	end
    
    if is>1 & is<=length(N) %partial inclusion of the last bin
        Npart = N(is) * ( smax-sc(is) ) / ( sc(is+1)-sc(is) );
        Nd=Nd+Npart;
        Nnew(is)=Nnew(is)-Npart; %partially deplete last bin
    end
