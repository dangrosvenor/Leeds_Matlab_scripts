...
%   $Revision: 5.28 $  $Date: 2000/06/19 15:18:04 $
...
if nargin < 2, 
   error('FZERO requires at least two input arguments'); 
end
...
if strcmp(calltype,'old')
   ...
   WarningStr=sprintf(...
   ['This syntax for calling FZERO is obsolete.  Use:\n',...
    'X = FZERO(FUN,X0,OPTIONS,P1,P2,...) instead.  See\n',...
    'the help entry for FZERO for more information on \n',...
    'syntax and usage.\n']);
   warning(WarningStr)
end
...
elseif isa(FunFcn,'function_handle') % function handle
   Ffcnstr = '';  % get the name passed in, but no converter function to get it yet
...
output.funcCount = fcount;
output.algorithm = 'bisection, interpolation';
fval = feval(FunFcn,b,varargin{:});
if trace > 1
   disp(['Zero found in the interval: [',num2str(savea),', ', num2str(saveb),'].']);
end

%------------------------------------------------------------------
function [tol, trace, calltype, FunFcn, otherargs]=parse_call(FunFcn,x,defaultopt,numargin,numargout,otherargs)
% old call: function b = fzero(FunFcn,x,tol,trace,varargin)
% new call: function b = fzero(FunFcn,x,options,varargin)

% New syntax has only 2 inputs: fzero(f,x)
if ( numargin < 3 )
   calltype = 'new';
   options = [];
else   % numargin >=3 
   ...
   elseif isstruct(thirdarg)   %  (numargout <= 1)
      ...
      calltype = 'new'; 
      options = thirdarg;
   elseif  length(thirdarg)==1 & numargin==3 %  (numargout <= 1)
      % fzero(f,x,tol)
      calltype = 'old';
      tol = thirdarg; 
      trace = 0;
   ...
   elseif isempty(thirdarg) & numargin==3  %  (numargout <= 1)
      % x = fzero(f,x,[])
      calltype = 'new';
      options = [];
   elseif numargin > 3 & isempty(thirdarg)
      % x = fzero(f,x,[],...)
...
[FunFcn,msg] = fcnchk(FunFcn,length(otherargs));
if ~isempty(msg)
   error(msg)
end
