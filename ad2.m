...
size(thcs);
siz=ans(2);
thi=atan( (log(thcs(1,siz)) - log(thcs(1,1))) / ( T(siz)-T(1) ) );
%thi=0;
%thi= atan( log(thcs(1,siz)) / T(siz) ) - atan(log(thcs(1,1)) / T(1) );
[xx,yy]=rotatedan(T,log(th),thi);
h=plot(xx,(yy),'r'); %pseudo-adiabat


[x,y]=rotatedan(T,log(thcs(i,:)),thi);
...
size(ts);
for i=1:ans(2)
   [x,y]=rotatedan(273.13+ts(i),log(thsfine),thi);
   h=plot( x,(y) );
...
[x,y]=rotatedan(273.13+Tdat,log(THdat),thi);
