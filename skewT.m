...
if fla==2
    ...
    FileName='c:/documents and settings/user/my documents/HIBISCUS/baurufield/soundings/ascii/1/04_02_06_12AED.txt';
end
...
figure;
pcs=[1000 900 800 700 600 500 400 300 200 100 50 30]; %mb
pfine=100.*[1000:-1:30];
ths=273:20:413;
thsfine=260:0.1:600;
ts=30:-20:-100;
tsfine=273+[30:-0.1:-100];

minT=tsfine(end)+273.13;
maxT=tsfine(1)+273.13;
minlogP=log10(pfine(1));
maxlogP=log10(pfine(end));
...
%xx=T.*cos(thi);
[xx yy]=skew(T,log10(p),thi,Y,minT,minlogP,maxlogP);
...
[x,y]=skew(tsfine,log10(100*pcs(i)),thi,Y,minT,minlogP,maxlogP);
h=plot( x,(y) ); %plot isobars
...
% %    h=plot( x,(y) );
% 
%     [x,y]=skew(T,log10(p),thi,Y,minT,minlogP,maxlogP);
%     h=plot( x,(y) );
...
%    h=plot( x,(y) ); %plot isotherms
    ...
    [x,y]=skew(273.13+ts(i),log10(pfine),thi,Y,minT,minlogP,maxlogP);
    h=plot( x,(y) ); %plot isotherms
...
axis( [xx(end)*1 xx(1)/1.3 yy(1)/1.3 yy(end)*1 ] ); %zoom into area of interest
set(gca,'XTicklabel','');
...
[x,y]=skew(273.13+Tdat,log10(100.*Pdat),thi,Y,minT,minlogP,maxlogP);
...
fprintf(1,'Cross-over height = %g m',dat(4,d));