...
pro=1; %flag to plot profiles
leg=0; %flag for legend
if pro==1
    timdiv=1; 
else
...
group=11; %switch for showing different sets of results
dualax=0; %switch =1 to use dual axis graphs
...
if pro==1
    hfig=figure('name','Combination plot','position',posit,'name',strcat('Combination Plot of col:',int2str(conm(1).c)));
else
...
minovall=0;
minovallR=0;  %for right hand axis
...
if pro==1
    ...
    for mc=1:sicm
        ...
        if pro==1
            sidat=size(pr(mc).p(:,conm(i).c),1);
            pser(i).ser(1:sidat,mc)=pr(mc).p(:,conm(i).c);
        else
            sidat=pser(i).ser(:,mc);
            size(conm(mc).c);
            sisum=ans(2);
            for isum=1:sisum   %do sum for lines with e.g. [62 63 64]
                if isum==1
                    pser(i).ser(:,mc)=ydata(:,conm(mc).c(isum));
                else
                    pser(i).ser(:,mc)=pser(i).ser(:,mc)+ydata(:,conm(mc).c(isum));
                end
            end
        end
        ...
        if lorr(mc)==0
            if max(pser(i).ser(:,mc))>maxovall
                maxovall=max(pser(i).ser(1:sidat,mc));
            end
            if min(pser(i).ser(:,mc))<minovall    
                minovall=min(pser(i).ser(1:sidat,mc));
            end
        else
            if max(pser(i).ser(:,mc))>maxovallR
                maxovallR=max(pser(i).ser(:,mc));  %max for right hand axis
            end
            if min(pser(i).ser(:,mc))<minovallR
                minovallR=min(pser(i).ser(:,mc));  %max for right hand axis
            end
        end
    end
end;
maxy=maxovall*1.1^sign(maxovall);
maxyR=maxovallR*1.1^sign(maxovallR);
miny=minovall*1.1^-sign(minovall);
minyR=minovallR*1.1^-sign(minovallR);
...
for k=1:1
    ...
    for mc=cmstart:sicm
        ...
        if lorr(mc)==0    %if is to be normally plotted on left
            if pro==1
                sidat=size(pr(mc).p(:,1),1);
                H1(mc).h = plot(pser(j).ser(1:sidat,mc),pr(mc).p(:,1));
            else
    ...
    if ans==1
        ...
        if pro==1
            set(AX,'xlim',[miny maxy]);
            set(AX(1),'ylim',[maxx 0]);
        else
            set(AX,'xlim',[0 maxx]);
    ...
    %if max(lorr)==1
        ...
        if pro==1
            set(hs(j).h,'ydir','reverse');
            set(hs(j).h,'xlim',[miny maxy]);
            set(hs(j).h,'ylim',[0 maxx]);
