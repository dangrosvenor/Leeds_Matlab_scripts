...
function [H1,ax1,ax2]=plotXY3(xdat,ydat,labs,nmark,lwidth,leglor,logflag,xlab,ylab,ylims,zline,dual) %xdat(1:n).x, ydat(1:n).y & labs(1:n).l nmark=no markers
...
for j=1:length(xdat)
     ...
     if dual==0
         ...
         [axx H2(j).h H3(j).h]=plotyy(xdat(2*j-1).x,ydat(2*j-1).y,xdat(2*j).x,ydat(2*j).y);
         if ydat(2*j-1).y(2)<ydat(2*j-1).y(1)
             set(axx(1),'ydir','reverse');
             set(axx(1),'ylim',[ydat(2*j-1).y(end) ydat(2*j-1).y(1)]);
         else
            set(axx(1),'ylim',[ydat(2*j-1).y(1) ydat(2*j-1).y(end)]);
         end
         
         if ydat(2*j).y(2)<ydat(2*j).y(1)
             set(axx(2),'ydir','reverse');
             set(axx(2),'ylim',[ydat(2*j).y(end) ydat(2*j).y(1)]);
         else
             set(axx(2),'ylim',[ydat(2*j).y(1) ydat(2*j).y(end)]);
         end
         set(H3(j).h,'LineStyle','none');
     ...
     if dual==0
         ...
         [axx H2(j).h H3(j).h]=plotyy(xdat(2*j-1).x(markpnts),ydat(2*j-1).y(markpnts),xdat(2*j).x(markpnts),ydat(2*j).y(markpnts));
     end
...
for j=1:length(xdat)
 ...
 if logflag==2 | logflag==12
	...
	for i=1:length(ynew)-1
        for j=1:n
            ind=(i-1)*n+j;
            cb(ind)=(ynew(i+1)-ynew(i))*j/n+ynew(i);
        end
	end
...
if exist('ax2') & logflag~=1 & logflag~=2;
    ...
    switch_objects_depth(gcf,ax1,ax2); %switchs order of figure children (ax and ax2) so that can zoom on ax
end