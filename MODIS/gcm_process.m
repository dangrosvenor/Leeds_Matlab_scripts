...
% ---- Low cloud -----
isccp = find( ~ (gcm_pfull >= 680e2) );
gcm_Nd_ice_screen2 = gcm_Nd_ice_screen;
gcm_Nd_ice_screen2(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_screen_isccp_low = squeeze_keep1( max(gcm_Nd_ice_screen2,[],2) ,inotkeep); %take the max


gcm_drop32 = gcm_drop3;
gcm_drop32(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_noCF_isccp_low = squeeze(max(gcm_drop32,[],2)); %max Nd and Re with no CF screening, except to remove the very low
...
gcm_Nd_ice_screen2 = gcm_Nd_ice_screen;
gcm_Nd_ice_screen2(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_screen_isccp_mid = squeeze_keep1( max(gcm_Nd_ice_screen2,[],2) ,inotkeep); %take the max


gcm_drop32 = gcm_drop3;
gcm_drop32(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_noCF_isccp_mid = squeeze(max(gcm_drop32,[],2)); %max Nd and Re with no CF screening, except to remove the very low
...
gcm_Nd_ice_screen2 = gcm_Nd_ice_screen;
gcm_Nd_ice_screen2(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_screen_isccp_high = squeeze_keep1( max(gcm_Nd_ice_screen2,[],2) ,inotkeep); %take the max


gcm_drop32 = gcm_drop3;
gcm_drop32(isccp) = NaN; %remove those that don't satisfy the criteria
gcm_Nd_max_noCF_isccp_high = squeeze(max(gcm_drop32,[],2)); %max Nd and Re with no CF screening, except to remove the very low



%% End of ISCCP classification for Nd etc.
