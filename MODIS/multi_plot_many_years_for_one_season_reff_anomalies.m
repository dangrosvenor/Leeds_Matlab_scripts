...
% overridden by thei driver script as yet


i_ocean_only_SCREEN = 0;

thresh_ndays_clim_DRIVER = 10; %Threshold for no. days in month that are rquired to include that month.
...
load_file_DRIVER = ['/home/disk/eos1/d.grosvenor/saved_misc_mat_files/MODIS_multi_year_multi_screening_reff_20150205T050509.mat'];


load(load_file_DRIVER);
...
iload_DRIVER=0;  %Prevents loading (for testing purposes, e.g. if have alrady loaded)
...
for iloop_year=1:length(set_year)
    ...
    %% Already loaded in the data for the required varaible
        ...
        for iscreen_set=1:length(set_screening)
            ...
            for iloop_MOD=1:length(set_month)
                ...
                % Make the climatology 
                  %Loop through all the years, except 2014
                  Nyears = length(Psave{icount_subplot})-1;
                  min_N = 1e9*ones(size(Nsave{icount_subplot}{1}));
                  clim_DRIVER = zeros([size(Nsave{icount_subplot}{1})]);
                for iclim_year=1:Nyears
                    min_N = min(min_N,Nsave{icount_subplot}{iclim_year});                    
                    clim_DRIVER = clim_DRIVER + Psave{icount_subplot}{iclim_year}/Nyears;                
                end
                inan = find(min_N<thresh_ndays_clim_DRIVER);
                clim_DRIVER(inan) = NaN; %remove points where we don't have a all years with >thresh days.
                
                
                       
                dat_modis = Psave{icount_subplot}{iloop_year} - clim_DRIVER;
                titlenam_driver = mtitle{icount_subplot};
...
%                copyobj(allchild(ax_save{icount_DRIVER}),ax_subplot);  
%  Year label
                title(remove_problem_chars(set_year{iloop_year}{1}(1:5)));
