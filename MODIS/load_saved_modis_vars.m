...
% filtering_data_get
%   is run to get CTH, etc.

irestrict_latlon = 1;
lat_restrict = [62 63]; lon_restrict = [27 28]; %Puijo_Sami : 62.91N, 27.66 E
lat_restrict = [-50 20]; lon_restrict = [-160 -60]; %VOCALS comparison used for CALIPSO CF comparison, etc.
%lat_restrict = [0 25]; lon_restrict = [-180 -150]; %Amy's Hawaii study
%lat_restrict = [-60 -40]; lon_restrict = [-180 180]; %Southern Ocean

%lat_restrict = [-40 0]; lon_restrict = [-140 -68]; %VOCALS to match mock L3 data for POLDER paper

%lat_restrict = [40 90]; lon_restrict = [-40 10]; %For Iceland volcano (Jan 2015)
%lat_restrict = [40 90]; lon_restrict = [-180 180]; %For Iceland volcano (Jan 2015)
