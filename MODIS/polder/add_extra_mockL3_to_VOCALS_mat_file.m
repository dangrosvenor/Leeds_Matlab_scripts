...
%If adding POLDER data, first load the POLDER data using load_process_PARASOL_gridded.m
...
%Now match to the time in the big (mutli-year) POLDER array in the VOCALS .mat
%file
...
% No need to flipdim since have searched for the MODIS values.
    ...
    eval_str = ['add_extra_mockL3_to_VOCALS_mat_file_FUNC(' modis_var_mockL3{ivar} ',''' savefile ''',''' modis_var_mockL3{ivar} ''',varname_ext,ilat,ilon,it2);'];
    eval(eval_str);
end
