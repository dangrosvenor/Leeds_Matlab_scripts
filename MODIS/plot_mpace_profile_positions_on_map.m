...
for iprofile=1:length(profile_set)
    ...
    for itime=1:size(time_profiles,1);
        ...
        %note - put vertical alignment as middle - will look wrong on the
        %figure, but will come out right when it is saved as an image
        %also choosing white for text makes it come out black... cyan works
        %and is almost white!

%        m_plot([mpace_lon_mapped(ii(1))],[mpace_lat_mapped(ii(1))],'ko','markersize',16,'markerfacecolor','r');
        m_text([mpace_lon_mapped(ii(1))],[mpace_lat_mapped(ii(1))],[num2str(nprof) 's'],'color','k','horizontalAlignment','center','verticalAlignment','Middle');     
            
%        m_plot([mpace_lon_mapped(ii(end))],[mpace_lat_mapped(ii(end))],'ko','markersize',16,'markerfacecolor','r');
%        m_text([mpace_lon_mapped(ii(end))],[mpace_lat_mapped(ii(end))],[num2str(nprof) 'e'],'color','c','horizontalAlignment','center','verticalAlignment','Middle');  %'Cap'
