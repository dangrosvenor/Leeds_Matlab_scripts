...
% *****************************************************   
    ...
    if ~exist('savevarname')  %for situations when there is only a TERRA file (y2000-2001)
        ...
        fprintf(1,'\n**** Warning - no saved file for imod=%d equivalent of %s. Need to set the file in filename_choose_saved_MODIS_files.m *****\n',imod,char(modis_data_case{1}));
        continue
    end
    ...
    switch data_type
        case {'L3 processed data','Nd vs SZA profiles at constant LAT','Mock L3 timeseries3'}
            %load in the variable required
                var_dat = load(savevarname{1},modis_varname);
                if length(fields(var_dat)) == 0 %Check to see if the variable was in the file
                    ...
                    switch modis_varname  %these variables are contained in a separate file
                        ...
                        otherwise %other variables
                            ...
                            %                          eval([modis_varname ';']); %will now give an error if it wasn't in the
                            %                           rethrow(load_error); %rethrow the error if not one of the above variables.

                            error(['*** Variable ' modis_varname ' is not in the file for ' char(modis_data_case{imod}) '...']);
                    end

                else %if we do have the data then give it its correct name
                   eval([modis_varname '=var_dat(1).' modis_varname ';']); 
                end
