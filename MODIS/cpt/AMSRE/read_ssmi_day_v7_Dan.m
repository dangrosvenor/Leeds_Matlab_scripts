...
% FOr SSMI the variables are as follows - thsese determine the order of
% the offsets:-
% time=squeeze(map(:,:,1,:));
% wspdMF=squeeze(map(:,:,2,:));
% vapor=squeeze(map(:,:,3,:));
% cloud=squeeze(map(:,:,4,:));
% rain=squeeze(map(:,:,5,:));
...
%note the negative offset for cloud (= -0.05) - is negative because they take
