...
titlenam = 'Precip rate timeseries';
xlab='Time (UTC)';
ylab='Domain mean of column max precip rate (mm day^{-1})';
xlims=0;
xlimits=[0 100];
...
%fileUM{idat} = 'xkqko_Precip_rate_.pp.nc.mat'; labs_UM(idat).l = '(xkqko) 100cm^{-3} RHcrit=0.7'; flag{idat}='load_mat'; pole_lat=70; pole_lon=278; idat=idat+1;
%fileUM{idat} = 'xkqkl_Precip_rate_.pp.nc.mat'; labs_UM(idat).l = '(xkqkl) 1000cm^{-3} RHcrit=0.7'; flag{idat}='load_mat'; pole_lat=70; pole_lon=278; idat=idat+1;
...
fileUM{idat} = 'xkqkw_rain_rate_.pp.nc'; labs_UM(idat).l = '100cm^{-3} RHcrit=0.7'; flag{idat}='load_mat'; pole_lat=70; pole_lon=278; idat=idat+1;
...
%    vars_in.file_rho = filename_rho;
    ...
    ydat_import(idat_driver).y = meanNoNan(lwp(:,:),2);
