function [out,std_dev,freq,Nii,inds,std_rel_norm,states] = calc_SW_forcing_cloud_fraction_combinations(dat,cf_low,cf_mid,cf_high,thresh)
...
states = NaN * ones(size(cf_low)); %store the cloud states (integers 1-8)
...
for i=1:size(sigs,1)
   ...
   % e.g. x > 0.1 would give the opposite result to -x > -0.1 (i.e. latter
   % would give x < 0.1).
   ...
   %Using normalized data to get an idea of relative variability
   dat2=(dat(ii)-min_dat)/(max_dat - min_dat);
%   dat2 = dat(ii) + 800;
   [me ,Ni, std_norm] = meanNoNan(dat2,1);
   
   std_rel_norm(i) = std_norm ./ me;
...
% Doesn't seem to work too well here, though
   ...
   states(ii) = i; %Store the cloud state integers (1-8).
