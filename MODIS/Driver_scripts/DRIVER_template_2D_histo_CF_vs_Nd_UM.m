...
if iuse_Rosenfeld_cf_bins==1
    dcf = dcf_vs_LWP(iLWP);
    Ybins_DRIVER = [dcf/2 dcf 2*dcf:dcf:1+dcf/2]; ichoose_Ybins_DRIVER=1; %test starting half a bin from zero - then continue normally
else
    dcf=0.1;
    dcf=0.2;
    dcf=0.01;
    Ybins_DRIVER = [-dcf/2:dcf:1+dcf/2]; ichoose_Ybins_DRIVER=1; %cloud fraction PD
end


switch Nd_bins
    case 'Orig'
        Xbins_DRIVER = [5:20:265]; ichoose_Xbins_DRIVER=1; %Nd
    case 'Equal log'
        Xbins_DRIVER = 10.^[log10(5):0.2:log10(500)]; ichoose_Xbins_DRIVER=1; %Nd
end
