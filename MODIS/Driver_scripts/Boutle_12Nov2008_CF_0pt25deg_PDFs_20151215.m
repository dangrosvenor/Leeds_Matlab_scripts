...
day_or_night = 'day 13th Nov'; %Select this for the PDFs
...
%        gcm_str = gcm_str_last_loaded;      
 ...
 % --------- Override flags for 2D PDF --------
        ...
        labs_import(idat_driver).l = ['GOES, \mu=' num2str(Y_mean_overall,'%.2f') ];
%        xlab_import = xlab;
...
% - 12th Nov case
% fileUM{idat} = '/xlhg-u/xlhgu_LWP_.pp.nc.mat'; fileUM_Nd{idat} = '/xlhg-u/xlhgu_Nd_.pp.nc'; labs_UM(idat).l = 'CASIM-Ndvar';  flag{idat} = 'load_mat'; pole_lat=70; pole_lon=284; 
%     line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.4 0.4]; marker_styleUM(idat).m='d'; idat=idat+1;
% fileUM{idat} = '/xlhg-v/xlhgv_LWP_.pp.nc.mat'; fileUM_Nd{idat} = '/xlhg-v/xlhgv_Nd_.pp.nc';labs_UM(idat).l = 'CASIM-Ndvar-0.1';  flag{idat} = 'load_mat'; pole_lat=70; pole_lon=284; 
%         line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0 0 0]; marker_styleUM(idat).m='v'; idat=idat+1;
% fileUM{idat} = '/xlhg-w/xlhgw_LWP_.pp.nc.mat'; fileUM_Nd{idat} = '/xlhg-w/xlhgw_Nd_.pp.nc';labs_UM(idat).l = 'CASIM-Ndvar-10';  flag{idat} = 'load_mat'; pole_lat=70; pole_lon=284; 
%         line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.8 0.4 0]; marker_styleUM(idat).m='^'; idat=idat+1;
% %fileUM{idat} = '/xlyd-m/xlydm_LWP_.pp.nc.mat'; fileUM_Nd{idat} = '/xlyd-w/xlydm_Nd_.pp.nc';labs_UM(idat).l = 'CASIM-Ndvar-10-aproc';  flag{idat} = 'load_mat'; pole_lat=70; pole_lon=284; 
...
%        gcm_str = gcm_str_last_loaded;        
 ...
 % --------- Override flags for 2D PDF --------
        ...
        labs_import(idat_driver).l = [labs_UM(idat_UM).l ', \mu=' num2str(Y_mean_overall,'%.2f') ];
        xlab_import = xlab;
        ylab_import = ylab;
%        ioverride_savePDF=1;
%        save_1D_pdfs;
