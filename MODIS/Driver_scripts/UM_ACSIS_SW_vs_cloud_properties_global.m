...
%These are used in DRIVER_calc_biases_for_regions.m
...
region_choice = 'US continent Jon Robson AMOC';
...
%clear time_choice; time_choice.time_range = [datenum('28-Mar-2010') datenum('29-Mar-2011 12:30')]; %extra year for revisions
...
%% SW direct and indirect forcing (surface) 
...
%% TOA SW direct and indirect forcing 
isave_plot_global_forcing_draft=1; %for paper draft
...
% indirect effect, but no direct), i.e. direct = indirect+direct - indirect
 ...
 var_UM = 'SW_up_clean_TOA';  %these seem ok RE the times in the files - i.e., the times match times_out. And are the same as for SW_up_TOA
 
    um_case=um_case_PI; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PI emissions
    dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];    
    dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);  
    [SW_TOA_nodirect_PI_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);
    
    um_case=um_case_PD; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PD emissions    
    dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
    dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);   
    [SW_TOA_nodirect_PD_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);   
    
    %For TOA we want these changes in the downward sense, so that net TOA
    %outgoing is negative - to ensure that negative forcings causing net
    %negative incoming SW.
    
    aerosol_effect_TOA_PI_ALL = - SW_TOA_PI_ALL + SW_TOA_nodirect_PI_ALL;    
    aerosol_effect_TOA_PD_ALL = - SW_TOA_PD_ALL + SW_TOA_nodirect_PD_ALL;
    direct_TOA_ALL = aerosol_effect_TOA_PD_ALL - aerosol_effect_TOA_PI_ALL;
    
% For indirect effect we do clean minus clear_clean.
% I.e., no aerosol effect with cloud effect - no aerosol effect without cloud effect
    %no direct or indirect effects
var_UM = 'SW_up_clean_clear_TOA'; 
    
    um_case=um_case_PI; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PI emissions
    dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];    
    dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);  
    [SW_TOA_clean_clear_PI_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);
    
    um_case=um_case_PD; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PD emissions    
    dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
    dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);   
    [SW_TOA_clean_clear_PD_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM); 
    
    cloud_effect_TOA_PI_ALL = - SW_TOA_nodirect_PI_ALL + SW_TOA_clean_clear_PI_ALL;    
    cloud_effect_TOA_PD_ALL = - SW_TOA_nodirect_PD_ALL + SW_TOA_clean_clear_PD_ALL;
    indirect_TOA_ALL = cloud_effect_TOA_PD_ALL - cloud_effect_TOA_PI_ALL;
...
clear time_choice_JJA; time_choice_JJA.time_range = [datenum('01-Jun-2009') datenum('31-Aug-2009 23:59')]; 
[out, time_out_CAL_JJA, time_inds_CAL_JJA, dtime_match_JJA] = get_time_range_of_array(array_in,gcm_time_matlab_CALIPSO_monthly,time_choice_JJA,dim);

array_in=[]; %just test to get the indices for now.
dim=NaN; %don't need dim if just getting the indices
[out_JJA, time_out_JJA, time_inds_JJA, dtime_match] = get_time_range_of_array(array_in,dat_global.time_ALL,time_choice_JJA,dim);
time_round_JJA = datestr(dat_global.time_ALL(time_inds_JJA(1)));


clear time_choice_DJF; time_choice_DJF.time_range = [datenum('01-Dec-2009') datenum('29-Feb-2010 23:59')]; 
[out, time_out_CAL_DJF, time_inds_CAL_DJF, dtime_match_DJF] = get_time_range_of_array(array_in,gcm_time_matlab_CALIPSO_monthly,time_choice_DJF,dim);

array_in=[]; %just test to get the indices for now.
dim=NaN; %don't need dim if just getting the indices
[out_DJF, time_out_DJF, time_inds_DJF, dtime_match] = get_time_range_of_array(array_in,dat_global.time_ALL,time_choice_DJF,dim);
time_round_DJF = datestr(dat_global.time_ALL(time_inds_DJF(1)));

switch season_str
    case '_JJA'
        time_inds_seasonal = time_inds_JJA;
    case '_DJF'
        time_inds_seasonal = time_inds_DJF;        
end
...
cloud_input2='CALIPSO';
[high_calipsoCF_PD_ALL,nT_CAL] = UM_ACSIS_global_low_cloud_FUNC('high',cloud_input2,um_case_PD,load_type,time_inds_seasonal,gcm_Plat2D_UM);

cloud_input2='MODIS';
[high_modisCF_PD_ALL,nT_modis] = UM_ACSIS_global_low_cloud_FUNC('high',cloud_input2,um_case_PD,load_type,time_inds_seasonal,gcm_Plat2D_UM);
...
%% Plot MODIS COSP optical depth etc
 ...
 switch load_type
    ...
    if iload_modis_liquid_cloud_fraction==1
        ...
        var_UM = 'modis_LWP'; %N.B. - this is weighted (multplied) by the liquid cloud fraction (2-452; modis_liquid_cloud_fraction)
        um_case=um_case_PD; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PD emissions
        dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
        dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
        [modis_cosp_lwp_PD,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);
        modis_cosp_lwp_PD(modis_mask==0) = NaN;
        lwp_cf_thresh = cf_thresh;
        %lwp_cf_thresh = 0.05;
        %lwp_cf_thresh = 0.1;
        idiv_lwp_CF = 0;
        if idiv_lwp_CF==1
            modis_cosp_lwp_PD = modis_cosp_lwp_PD ./ liqCF_COSP_MODIS_PD_ALL; %Divide by liquid CF to get in-cloud reff  
            modis_cosp_lwp_PD(liqCF_COSP_MODIS_PD_ALL < lwp_cf_thresh) = NaN;
        end
...
%    dat_modis  = meanNoNan(low_modisCF_PD_ALL(:,:,:),3); var_UM = 'Time mean MODIS COSP low CF (Present Day)';
 ...
 %% Alternative LWP to that used in originaal paper (2-391 LWP calculated by
 %%model from radiation - daytime only?
 
 var_UM = 'LWP_2-391'; 
 um_case=um_case_PD; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PD emissions
 dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
 dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
 [LWP_2_391_PD,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM); 
 lwp_cf_thresh = cf_thresh;
        %lwp_cf_thresh = 0.05;
 lwp_cf_thresh = 0.1;
 idiv_lwp_CF = 1;
 cf = liqCF_COSP_MODIS_PD_ALL;
 cf = low_CF_PD_ALL;
 cf = total_CF_PD_ALL;
 if idiv_lwp_CF==1
     LWP_2_391_PD = LWP_2_391_PD ./ cf; %Divide by liquid CF to get in-cloud reff
     LWP_2_391_PD(cf < lwp_cf_thresh) = NaN;
 end
 

 
 %% Comparison of MODIS LWP to COSP LWP
 isave_plot_LWP_modis=0;
 model_lwp_var = 'Model COSP MODIS LWP';
 model_lwp_var = 'Model LWP 2-391';
 modis_CTH_screening = 'none';
 %modis_CTH_screening = '3.2km';
 %modis_CTH_screening = '3.2km, CF>80';
 %modis_CTH_screening = 'AMSR-E';
 lwp_day_night = 'average';
 lwp_day_night = 'day';
 UM_ACSIS_global_SUBPLOT_commands_LWP_eval_MODIS
 


 %% Comparison of liquid cloud fraction for COSP MODIS Liq CF vs MODIS
 UM_ACSIS_global_SUBPLOT_commands_cloud_eval_MODIS
...
%% Offline Surface SW indirect effect calcs
% e.g. fig:est_contrib, etc.
...
diurnal_SW = 'average'; %which calipso data to use.
%diurnal_SW = 'day';
...
% MatlabSecLab:toa_calc
isave_plot_SWTOA_calcs_draft = 0;

time_mean=0; 
%time_mean=1; %=1 Used in original paper (Grosvenor and Carslaw 2021). Also
%set GC20=1
GC20=0;

A_clear = 0.09; %Orig value
%A_clear = 0.19; %Orig value

%cf_type only applies to time_mean==1
nd_type='all times';
...
%cf_type only applies to time_mean==0 for now.
cf_type = 'CALIPSO low'; %used in original paper.
cf_type = 'Model total';
%cf_type = 'Model low';
...
N1_totCF = N1;
W1_totCF = LWP_PD_ALL(:,:,it_sw)./total_CF_PD_ALL(:,:,it_sw); %Need to supply the in-cloud LWP, so divide by CF
cf_min = 0.01;
W1_totCF(total_CF_PD_ALL(:,:,it_sw)<cf_min)=NaN; %regions with no cloud - set W to NaN since have no cf to divide by
N1_totCF(total_CF_PD_ALL(:,:,it_sw)<cf_min)=NaN; %Do the same for Nd for consistency

switch time_mean
    case 0               
        
        %lwp =  lwpic_model_all_times_restrict_save; %was using this
        %before, but is patchy
        ...
        switch cf_type
            ...
            case 'Model low'
                cf = low_CF_PD_ALL;  
                lwp = W1;
                %lwp = W1_totCF; %It actaully seems that the low CF leads to the high values seen in VOCALS region
                %when using low CF. low CF can actually be higher than
                %total CF! Maybe comes from a different part of the UM code
                %diags? Swithcing in the LWP calculated using total CF
                %doesn't fix it.
        ...
        %SW_in calculated earlier. SW_in = SW_in_PD_ALL(:,:,it_sw); %This is the TOA downward SW        
    ...
    case 1
        if GC20==1
            ...
            %do the time-averages - should do these as monthly really
            %Have 325 time indices because we have 27 hourly output.
            %365*24/27 = 324.4444
            %So could split into 365/12 days per month, or 325/12=27.1 time indices
            cf2=cf;
            lwp2=lwp;
            nd2=nd;
            %sw2=sw;
            ...
            for i=1:12
                i0 = (i-1)*N+1;
                i1 = i0 + N -1;
                cf(:,:,i) = meanNoNan(cf2(:,:,i0:i1),3);
                nd(:,:,i) = meanNoNan(nd2(:,:,i0:i1),3);
                lwp(:,:,i) = meanNoNan(lwp2(:,:,i0:i1),3);
                sw(:,:,i) = meanNoNan(SW_in(:,:,i0:i1),3);
            end
            
            sw = fac_SWin * sw .* trans;
...
% irestrict_domain_DRIVER=1;
% %var_UM 
% subtitle_str= ['SW in TOA'];
% tit_str_clean='';
% figure
% dat_modis = meanNoNan(sw,3);
% UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
% lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
% %caxis([0 1]);
% 
...
% tit_str_clean='Model values';
% figure
% dat_modis = meanNoNan(nd,3);
% UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
% lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
% caxis([0 300]);

% Plots from above SWTOA calcs

ioverride_LAT_plots=0;
irestrict_domain_DRIVER=0;
subtitle_str =['SW TOA model ' str_type_calc ' calc'];
tit_str_clean='Model values';
figure
...
ioverride_LAT_plots=0;
irestrict_domain_DRIVER=0;
subtitle_str = ['SW TOA actual'];
tit_str_clean='Model values';
figure
dat_modis = SW_TOA_model_save;
UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
caxis([0 200]);
...
% ioverride_LAT_plots=0;
% irestrict_domain_DRIVER=1;
% subtitle_str =['SW TOA percentage bias of calc using ' str_type_calc ' model fields vs actual model SW TOA'];
% figure
...
% ioverride_LAT_plots=0;
% irestrict_domain_DRIVER=1;
% subtitle_str =['SW TOA model over calc ratio using ' str_type_calc ' model fields vs actual model SW TOA'];
% figure
% dat_modis = ( SW_TOA_model_save ./ SWTOA_calc_model_mean );
% UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
% lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
% caxis([0 2]);
...
%% 3 panel plot for the Nd, LWP and Cf SW bias abs contributions.
%fig:est_contrib_SWTOA_abs
...
%% PI to PD % diffs of SWTOA and indirect effect
isave_plot_global_SW=1;
...
%Need percentage increase in aerosol effect (DRE) and cloud effect (CRE).
%Actually this should be expressed as a % of the PI SWTOA since this is what is
%used in timeseries. So forcing/SW_TOA_PI. This would give the % change in
%SW due to the forcing - using % change allows for if there is a different
%baseline SW.
% so set um_data and sat_data appropriately.
% And do for total SWTOA change too.

SW_TOA_PI_mean = meanNoNan(SW_TOA_PI_ALL,3);
SW_TOA_indirect_mean = meanNoNan(indirect_TOA_ALL,3);

um_data = SW_TOA_PI_mean - SW_TOA_indirect_mean; %forcing is negative for a cooling, which actually means more SW TOA outgoing; so subtract
sat_data = SW_TOA_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_indirect_mean;
var_UM = 'Percentage change in SW TOA due to indirect forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% SWTOA indirect change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
%SW_TOA_PI_mean = meanNoNan(SW_TOA_PI_ALL,3);
SW_TOA_direct_mean = meanNoNan(direct_TOA_ALL,3);

um_data = SW_TOA_PI_mean - SW_TOA_direct_mean; %forcing is negative for a cooling, which actually means more SW TOA outgoing; so subtract
sat_data = SW_TOA_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in SW TOA due to direct forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% SWTOA direct change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
um_data = SW_TOA_PI_mean - SW_TOA_direct_mean - SW_TOA_indirect_mean; %forcing is negative for a cooling, which actually means more SW TOA outgoing; so subtract
sat_data = SW_TOA_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in SW TOA due to direct+indirect forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% SWTOA indirect+direct change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
if isave_plot_global_SW==1
    ...
    var_Latex = 'dSWTOABoth';
    DRIVER_calc_biases_for_regions
end


% Overall SW change effect

%SW_TOA_PI_mean = meanNoNan(SW_TOA_PI_ALL,3);
SW_TOA_PD_mean = meanNoNan(SW_TOA_PD_ALL,3);

um_data = SW_TOA_PD_mean; %forcing is negative for a cooling, which actually means more SW TOA outgoing; so subtract
sat_data = SW_TOA_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in SW TOA (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% SWTOA change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
%% As above, but for the surface :- PI to PD % diffs of SW surface and indirect effect
...
SW_surf_PI_mean = meanNoNan(SW_down_PI_ALL,3);
SW_surf_indirect_mean = meanNoNan(indirect_ALL,3);

um_data = SW_surf_PI_mean + SW_surf_indirect_mean; %forcing is in downward sense, so need to add for surface downwelling SW
sat_data = SW_surf_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_indirect_mean;
var_UM = 'Percentage change in surface SW due to indirect forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% Surface SW indirect change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
%SW_TOA_PI_mean = meanNoNan(SW_TOA_PI_ALL,3);
SW_surf_direct_mean = meanNoNan(direct_ALL,3);

um_data = SW_surf_PI_mean + SW_surf_direct_mean; %
sat_data = SW_surf_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in surface SW due to direct forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% Surface SW direct change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
um_data = SW_surf_PI_mean + SW_surf_direct_mean + SW_surf_indirect_mean; %
sat_data = SW_surf_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in surface SW due to direct+indirect forcing (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% Surface SW direct+indirect change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
if isave_plot_global_SW==1
    ...
    var_Latex = 'dSWSurfBoth';
    DRIVER_calc_biases_for_regions
end

% Overall (actual) SW changne effect

%SW_TOA_PI_mean = meanNoNan(SW_TOA_PI_ALL,3);
SW_surf_PD_mean = meanNoNan(SW_down_PD_ALL,3);

um_data = SW_surf_PD_mean; %forcing is negative for a cooling, which actually means more SW TOA outgoing; so subtract
sat_data = SW_surf_PI_mean; %for the stats - special case where need regional calcs of two things first before doing percentage diff/change

dat_modis = 100*(um_data - sat_data) ./ sat_data;
%dat_modis = SW_TOA_direct_mean;
var_UM = 'Percentage change in surface SW (PD minus PI)';
%tit_str_clean='% f_c change';
subtitle_str='% Surface SW change';
%dat_modis(dat_modis>999)=999; %to deal with gridpoints with Inf values from where f0_mean=0
...
%% Contributions from Nd, CF, LWPic for SWTOA instead of surface
% Set values to use for N0, W0, f0, etc.

cf_type_TOA = 'Low';
cf_type_TOA = 'Total';
switch cf_type_TOA
    case 'Low'
        [f0_TOA,N0_TOA,W0_TOA,f1_TOA,N1_TOA,W1_TOA,N0_2_TOA,W0_2_TOA,N1_2_TOA,W1_2_TOA] = calc_cloud_variables_for_SW_TOA_calc(low_CF_PI_ALL,Nd_PI_ALL,LWP_PI_ALL,low_CF_PD_ALL,Nd_PD_ALL,LWP_PD_ALL,cf_min);
    case 'Total'
        [f0_TOA,N0_TOA,W0_TOA,f1_TOA,N1_TOA,W1_TOA,N0_2_TOA,W0_2_TOA,N1_2_TOA,W1_2_TOA] = calc_cloud_variables_for_SW_TOA_calc(total_CF_PI_ALL,Nd_PI_ALL,LWP_PI_ALL,total_CF_PD_ALL,Nd_PD_ALL,LWP_PD_ALL,cf_min);
end

%SW_in = SW_in_PD_ALL(:,:,it_sw); %This is the TOA downward SW

%A_clear = 0.09; %set earlier
[SW_estimated_forcing_cf_PIPD_TOA_timemean, SW_estimated_forcing_Nd_PIPD_TOA_timemean, SW_estimated_forcing_lwp_PIPD_TOA_timemean, ...
    SW_PI_TOA_est, SW_PD_TOA_est] ...
    = calc_SW_contribution_TOA(f0_TOA,W0_TOA,N0_TOA,f1_TOA,W1_TOA,N1_TOA,W0_2_TOA,N0_2_TOA,W1_2_TOA,N1_2_TOA,SW_in_PD_ALL,cf_min,A_clear);

%how was done earlier in section - search for MatlabSecLab:toa_calc
%[Ac_calc_model,tau_calc_model,A_calc_model,SWTOA_calc_model] = calc_SW(cf,lwp,nd/1e6,sw,A_clear,trans,cf_min,0,NaN,i_Liu); 
%SWTOA_calc_model_mean = meanNoNan(SWTOA_calc_model,3);

%% Tests of SWTOA calculations to make sure they are similar to actual, etc.

    var_UM = 'Actual SWTOA PD'; subtitle_str = var_UM;   
    dat_modis = meanNoNan(SW_TOA_PD_ALL,3);
    %run plotting script
    figure
    UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
    ...
    var_UM = 'Estimated SWTOA PD'; subtitle_str = var_UM;    
    dat_modis = meanNoNan(SW_PD_TOA_est,3);
    %run plotting script
    figure
    UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
    lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
    ...
    var_UM = 'Estimated earlier SWTOA PD'; subtitle_str = var_UM;    
    dat_modis = meanNoNan(SWTOA_calc_model,3);
    %run plotting script
    figure
    UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
    lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
...
%% Comparing the actual vs estimated forcing for TOA
...
isave_plot=0;
    ...
    var_UM = 'Total estimated change in SWTOA net downwelling'; subtitle_str = var_UM;
    SW_estimated_forcing_total_TOA = - (meanNoNan(SW_PD_TOA_est,3) - meanNoNan(SW_PI_TOA_est,3) ); %negative 
    dat_modis = meanNoNan(SW_estimated_forcing_total_TOA,3);
    um_data = dat_modis;
    sat_data = dat_modis;
    %run plotting script
    figure
...
%     LAT_val = LAT_val_DRIVER2;
%     LON_val = LON_val_DRIVER2;
%     [iregion_lin,iregion_lin_edges,SWTOA_indirect_estimate_region]=get_lat_lon_irregular_with_time(1,LAT_val,LON_val,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,dat_modis);
%     SWTOA_indirect_estimate_region_mean = meanNoNan(SWTOA_indirect_estimate_region(:),1);
%     
    
    var_UM = 'Actual indirect change (forcing) in SWTOA'; subtitle_str = var_UM;
    dat_modis = meanNoNan(indirect_TOA_ALL,3);
    um_data = dat_modis;
    sat_data = dat_modis;
    %run plotting script
    figure
    ...
    if isave_plots_partioning==1
        ...
        var_Latex = 'dSWTOA';
        DRIVER_calc_biases_for_regions
        stats_actual = region_biases;
    end
    
    for i=[1:4 8]
        fprintf(1,'%f ',stats_actual{i}.me_true);      
    end
    fprintf(1,'\n');  
    
    for i=[1:4 8]
        fprintf(1,'%f ',stats_estimated{i}.me_true);      
    end
    fprintf(1,'\n'); 
    
    iplot_comparisons=0;

    if iplot_comparisons==1
    
    %restrict to the region of interest
%     LAT_val = LAT_val_DRIVER2;
%     LON_val = LON_val_DRIVER2;
%     [iregion_lin,iregion_lin_edges,SW_indirect_actual_region_TOA]=get_lat_lon_irregular_with_time(1,LAT_val,LON_val,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,dat_modis);
%     SW_indirect_actual_region_mean_TOA = meanNoNan(SW_indirect_actual_region_TOA(:),1)
%     
%     SW_estimate_regional_mean_bias_TOA = 100* ( 1 - SW_indirect_estimate_region_mean_TOA./SW_indirect_actual_region_mean_TOA);
%     
    
    
    var_UM = 'Ratio estimated vs actual change';
    dat_modis = meanNoNan(SW_estimated_forcing_total_TOA,3) ./ meanNoNan(indirect_TOA_ALL,3);
    %run plotting script
    figure
    UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
    lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
    ...
    dat_modis = 100*(meanNoNan(SW_estimated_forcing_TOA,3) ./ meanNoNan(indirect_TOA_ALL,3) - 1);
    %run plotting script
    figure
    UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
    lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
...
%% Contributions from Nd, CF, LWPic for SWTOA instead of surface - plot and save data for regions / table etc.

var_UM = 'Change in SWTOA due to CF PI+PD average';
subtitle_str = 'f_c contribution';
dat_modis = SW_estimated_forcing_cf_PIPD_TOA_timemean;
um_data = dat_modis;
sat_data = dat_modis;
icoarse_grain=1; M_coarse_grain=3; N_coarse_grain=3;
...
caxis([-5 5]);
    ...
    if isave_plots_partioning==1       
        ...
        saveas_ps_fig_emf(gcf,[savename],'',0,0,0,'',[],0,opts);
%        close(gcf);        
        var_Latex = 'SWTOAforcingCFPIPD';         
        DRIVER_calc_biases_for_regions
    end 

    var_UM = 'Change in SWTOA due to LWP PI+PD average';
subtitle_str = 'LWP_{ic} contribution';
dat_modis = SW_estimated_forcing_lwp_PIPD_TOA_timemean;
um_data = dat_modis;
sat_data = dat_modis;
%run plotting script
figure
...
var_UM = 'Change in SWTOA due to Nd PI+PD average';
subtitle_str = 'N_d contribution';
dat_modis = SW_estimated_forcing_Nd_PIPD_TOA_timemean; 
um_data = dat_modis;
sat_data = dat_modis;
...
%fig:est_contrib - individual data for this plotted and stored in .mat files here
iplot_box_on_map=0;
