% Map plots for all times for UM
...
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
     ...
     lwp = max(dBZ_3D,[],2);
     siz=size(lwp);
     lwp=reshape(lwp,siz(1),sqrt(siz(3)),sqrt(siz(3)));
    
    
    for it_driver=16:16 %1:length(time_matlab)  %4:nt_driver
        %12=23:00 UTC on 12th, 16 = 07:00UTC on 13th, 16 = 02:00 LST on 13th;  22=19:00 UTC on 13th; 
        ...
        titlenam_driver = ['Max radar reflectivity for ' time_str ' for ' labs_UM(idat_UM).l];
        units_str_plot = 'dBZ';
