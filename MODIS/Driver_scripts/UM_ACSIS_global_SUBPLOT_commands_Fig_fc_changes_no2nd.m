...
fig_3panel_savename = 'fc_changes_no2nd';
...
var_Latex='dlowCFNoSec';
switch fc_change_type
    case 'Absolute'
        cbar_title{1} = 'f_c change (PD minus PI)';
        caxis_range = [-0.02 0.02];
    case 'Percentage'
        cbar_title{1} = '% f_c change';
        caxis_range = [-15 15];
        caxis_range = [-5 5];
end
...
%% ----  Standard runs with 2nd indirect effect ---------------------------
    ...
    switch fc_change_type
        ...
        case 'Percentage'
            dat_modis = 100*((f1_mean - f0_mean) ./ f0_mean); %var_UM = 'Percent change in cloud fraction, aerosol not affecting autoconversion (PD minus PI)';
    end
...
%% ----  No 2nd indirect effect runs ---------------------------   
    ...
    switch fc_change_type
        case 'Absolute'
            dat_modis = meanNoNan(f1_no2nd,3) - meanNoNan(f0_no2nd,3); %var_UM = 'Absolute change in cloud fraction (PD minus PI)';  
        case 'Percentage'
            dat_modis = 100*(f1_mean_no2nd - f0_mean_no2nd) ./ f0_mean_no2nd; %var_UM = 'Percent change in cloud fraction, aerosol not affecting autoconversion (PD minus PI)';
    end
...
%     dat_modis = meanNoNan(indirect_ALL,3);  
    ...
    %indirect effect run
    um_data = f1_mean_no2nd;
    sat_data = f0_mean_no2nd; 
