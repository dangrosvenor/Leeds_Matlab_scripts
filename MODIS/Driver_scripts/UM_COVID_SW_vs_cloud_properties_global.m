...
var_UM_DRIVER = 'SW_up_TOA';
var_UM_DRIVER = 'Nd_cloud_top';
...
run_set = 'COVID nudged runs PI PD';
run_set = 'COVID nudged runs E3A1';
run_set = 'COVID nudged runs E3A1 to May 2020';
%run_set = 'COVID nudged runs Piers emissions to May 2020';
...
switch run_set
  ...
  case 'COVID nudged runs PI PD';
        um_case_PI = 'u-bt679'; %
        um_case_PD = 'u-bt595';

 case 'COVID nudged runs E3A1';
        um_case_PI = 'u-bu156'; %COVID (lower aerosol)
        um_case_PD = 'u-bu008'; %Control (higher aerosol)
        
 case 'COVID nudged runs E3A1 to May 2020';
        um_case_PD = 'u-bu732'; %COVID (lower aerosol)
        um_case_PI = 'u-bu733'; %Control (higher aerosol)       
      
case 'COVID nudged runs Piers emissions to May 2020';
    um_case_PI = 'u-bw683'; %Control (higher aerosol)           
    um_case_PD = 'u-bw684'; %COVID (lower aerosol)
...
clear time_choice; time_choice.time_range = [datenum('01-Jan-2020') datenum('31-Mar-2020 23:59')];
clear time_choice; time_choice.time_range = [datenum('01-Mar-2020') datenum('31-Mar-2020 23:59')];
clear time_choice; time_choice.time_range = [datenum('15-Mar-2020') datenum('31-Mar-2020 23:59')];
clear time_choice; time_choice.time_range = [datenum('15-Mar-2020') datenum('31-May-2020 23:59')];
...
%% SWTOA vs CERES. Plus change in SW out TOA. Used for COVID too.
...
%    dat_modis = meanNoNan(SW_TOA_PI_ALL,3);
    ...
    SW_TOA_up_PD_minus_PI_mean = dat_modis;
    ...
    lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
    caxis([-12 12]);  
    title('SWTOA_{PD}-SWTOA_{PI} (W m^{-2})');
...
if iceres_data==1   
 ...
 %% Calculate the CERES inter annual std dev for climatology as map - for COVID runs
 siz = size(obs_annual_map_DeepC);
 i_detrend=0;
 if i_detrend==1
     x_trend_map = repmat(trend_dat_map{1}.x,[1 siz(2) siz(3)]);
     co_trend_map = repmat(trend_dat_map{it_trend}.coeffs(1,:,:),[siz(1) 1 1]);
     co2_trend_map = repmat(trend_dat_map{it_trend}.coeffs(2,:,:),[siz(1) 1 1]);
     trend_map = co_trend_map + x_trend_map .* co2_trend_map;
     dat = obs_annual_map_DeepC - trend_map;
 end
 %[ceres_me,ceres_N,ceres_std] = meanNoNan(dat,1);
 [ceres_me,ceres_N,ceres_std] = meanNoNan(obs_annual_map_DeepC,1);
 dat_modis = ceres_std;
 %dat_modis = ceres_me;
 %dat_modis = meanNoNan(dat,1);
 %dat_modis = meanNoNan(trend_map,1);
 figure
 ...
 % plot the PD minus PI SW TOA signal as a fraction of the std dev of the
 % CERES climatology
 figure
 dat_modis = SW_TOA_up_PD_minus_PI_mean ./ ceres_std;
 UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
 lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
 
 title('SWTOA_{PD}-SWTOA_{PI} expressed as number of sigma of the CERES inter-annual variability for JFM');
 
 
 figure
 dat_modis = SW_TOA_up_PD_minus_PI_mean;
 UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
 lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));

 %% Nd from cloud top variable
 ...
 dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
 dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
 [Nd_cloud_top_in_PD_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);
 
 um_case=um_case_PI; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PI emissions
 dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
 dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
 [Nd_cloud_top_in_PI_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);

 var_UM = 'Nd_cloud_top_weight';
 
 um_case=um_case_PD; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PD emissions
 dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
 dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
 [Nd_cloud_top_weight_PD_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);
 
 um_case=um_case_PI; pole_lat=45.0; pole_lon=145.0; run_type = 'umglaa'; icoarse=0; ivar_dir=1; %global ACSIS PI emissions
 dirUM = ['/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/' um_case];
 dat_global = UM_load_merged_netCDF(dirUM,var_UM,run_type,load_type);
 [Nd_cloud_top_weight_PI_ALL,nT] = UM_get_time_data_mat_nc(dat_global,time_inds,load_type,gcm_Plat2D_UM);

 Nd_cloud_top_PD_ALL2 = Nd_cloud_top_in_PD_ALL ./ Nd_cloud_top_weight_PD_ALL;
 Nd_cloud_top_PI_ALL2 = Nd_cloud_top_in_PI_ALL ./ Nd_cloud_top_weight_PI_ALL;
 
 mean_Nd_cloud_top_PI_ALL2 = meanNoNan(Nd_cloud_top_PI_ALL2,3);
 mean_Nd_cloud_top_PD_ALL2 = meanNoNan(Nd_cloud_top_PD_ALL2,3);
 
 figure
 dat_modis = mean_Nd_cloud_top_PD_ALL2 - mean_Nd_cloud_top_PI_ALL2;
 dNd_cloud_top_mean = dat_modis;
 UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
 lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
 title('Nd_{PD} - Nd_{PI} (cm^{-3})');
 
 figure
 dat_modis = 100*(mean_Nd_cloud_top_PD_ALL2 - mean_Nd_cloud_top_PI_ALL2) ./ mean_Nd_cloud_top_PI_ALL2;
 dNd_percent_cloud_top_mean = dat_modis;
 UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
 lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
 title('Nd_{PD} - Nd_{PI} (%)');
 
 
 %% MODIS interannual variability
  years_obs_modis = years_MODIS2; %from load script 
  dat_obs = Nd_monthly_MODIS;
  %if iscreen_land==1
  %    [dat_tmp,lmask_out] = SW_Robson_obs_land_mask_FUNC(dat_obs,gcm_Plat2D_AMSRE,gcm_Plon2D_AMSRE);
  %end
  
  %[obs_monthly_box_modis,obs_annual_box_modis,obs_annual_map_modis] = ACSIS_Robson_paper_process_obs(dat_obs,gcm_Plat2D_AMSRE,gcm_Plon2D_AMSRE,LAT_val,LON_val,years_obs_modis,gcm_area_AMSRE,season);
  
  %run this first to get the trends, if want to detrend :-
  %ACSIS_Robson_paper_plot_trend_maps_obs
if i_detrend==1
 siz = size(obs_annual_map_modis);
 x_trend_map = repmat(trend_dat_map{1}.x,[1 siz(2) siz(3)]);
 co_trend_map = repmat(trend_dat_map{it_trend}.coeffs(1,:,:),[siz(1) 1 1]);
 co2_trend_map = repmat(trend_dat_map{it_trend}.coeffs(2,:,:),[siz(1) 1 1]);
 trend_map = co_trend_map + x_trend_map .* co2_trend_map;
 dat = obs_annual_map_modis - trend_map;
 [modis_me,modis_N,modis_std] = meanNoNan(dat,1);
end
 
 [modis_me,modis_N,modis_std] = meanNoNan(obs_annual_map_modis,1);
 dat_modis = modis_std;
 dat_modis = griddata(gcm_Plat2D_AMSRE,gcm_Plon2D_AMSRE,dat_modis,gcm_Plat2D_UM,gcm_Plon2D_UM);;
 modis_std_regridded = dat_modis;
 %dat_modis = modis_me;
 %dat_modis = meanNoNan(dat,1);
 %dat_modis = meanNoNan(trend_map,1);
 figure
...
dat_modis =  dNd_cloud_top_mean ./ modis_std_regridded;
UM_ACSIS_LWP_global_vs_nest_quick_plot_commands_global
lb_map = lbmap(16,'brownblue'); colormap(flipdim(lb_map,1));
 
title('Nd_{PD}-Nd_{PI} expressed as number of sigma of the MODIS inter-annual variability for JFM');

 %
 
 
 %% COVID - examine Piers's SO2 emissions files from Matthew (Youngsub)
 
dir_SO2 = '/home/disk/eos10/d.grosvenor/UM/12Nov2008_Boutle/u-bw683/';
file_SSP245 = [dir_SO2 'SO2_all_low_anthropogenic_2014_2101_time_series.nc3'];
file_Matthew = [dir_SO2 'SO2_anthro_combined_anthropogenic_2019_2024_time_series.nc3']; 
nc_Matt = netcdf(file_Matthew);
nc_SSP = netcdf(file_SSP245);
 
SO2_Matt = nc_Matt{'emissions_SO2_anthro_agricultural_kgSO2'}(:) + nc_Matt{'emissions_SO2_anthro_energy_kgSO2'}(:)...
    + nc_Matt{'emissions_SO2_anthro_industrial_kgSO2'}(:) + nc_Matt{'emissions_SO2_anthro_rco_kgSO2'}(:)...
    + nc_Matt{'emissions_SO2_anthro_ship_kgSO2'}(:) + nc_Matt{'emissions_SO2_anthro_solvents_kgSO2'}(:)...
    + nc_Matt{'emissions_SO2_anthro_transport_kgSO2'}(:) + nc_Matt{'emissions_SO2_anthro_waste_kgSO2'}(:);
time_Matt = nc_Matt{'time'}(:);

SO2_SSP = nc_SSP{'emissions_SO2_all_low_kgSO2'}(:);
time_SSP = nc_SSP{'time'}(:);

SO2_Matt_timeseries = meanNoNan(meanNoNan(SO2_Matt,3),2);
SO2_SSP_timeseries = meanNoNan(meanNoNan(SO2_SSP,3),2);

figure
plot(time_Matt,SO2_Matt_timeseries,'b-','linewidth',3); hold on
plot(time_SSP,SO2_SSP_timeseries,'r'); hold on
xlabel('Days since 01-Jan-2015');
grid on
title('Nd_{PD}-Nd_{PI} expressed as number of sigma of the MODIS inter-annual variability for JFM');
 
ssp_interp = interp1(time_SSP,SO2_SSP_timeseries,time_Matt);
figure
dSO2 = SO2_Matt_timeseries - ssp_interp;
pdiff = (dSO2 ./ ssp_interp)*100;
t_years = time_Matt/360 + 2015;
plot(t_years,pdiff,'b--','linewidth',1); hold on
xlabel('Year');
ylabel('% difference Matt minus SSP245');
grid on
...
%% Make the 3-panel plot of the Nd, LWPic and CF changes
...
tit_str_clean ='Calculated, all perturbed';
...
tit_str_clean = 'Calculated, sum of individual';
