...
% --- Run UM_SW_calc_whole_field.m after this to do the SW calcs
...
%Make indices saying where each variable will be in UM_dat_multi
for ivar=1:length(var_to_calc_multi)
   eval_str = ['i' var_to_calc_multi{ivar} '=ivar;'];
   eval(eval_str);
end
...
% for CF will need to process a .mat file for the CF as for Lwp, etc.
    ...
    UM_time_in.time_specific =  datenum('13-Nov-2008 17:00');
    UM_time_in.time_specific =  datenum('13-Nov-2008 20:00');
        
    %UM_time_in.time_specific =  [datenum('12-Nov-2008 22:00') datenum('13-Nov-2008 12:00')];
    %UM_time_in.time_range =[ datenum('13-Nov-2008 06:00')  datenum('14-Nov-2008 00:00') ];
    
    
    %% Set the flags
    UM_time_in.action='';  %'timeseries domain means' %save_all  - set to '' for 2D fields
    UM_time_in.noplot = 0; %set to 1 for timeseries domain means for faster processing
    UM_time_in.iclose_figs=0; %whether to close the figures after plotting
