...
ylab='Liquid+Rain Water Path (g m^{-2})';
...
nmark=6; %-1 means that all points have markers. Otherwise only plot the number specified.
...
%         vars_in.file_rho = [dirUM_i fileUM_rho{idat_UM}]; %filename_rho;
    ...
    lwp=permute(lwp,[3 1 2]);
         ...
         vars_in.var = 'RWP';
         vars_in.flag = 'load_mat';
         vars_in.file_lwp =  [remove_character(filename,'VAR_NAME','RWP') '.mat'];
%         vars_in.file_rho = [dirUM_i fileUM_rho{idat_UM}]; %filename_rho;
         vars_in.pole_lat = pole_lat;
         vars_in.pole_lon = pole_lon;
         vars_in.time_in = []; %set to this for all times
