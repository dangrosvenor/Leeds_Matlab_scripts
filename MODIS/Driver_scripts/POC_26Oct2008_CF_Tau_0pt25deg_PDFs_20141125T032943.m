...
% area with an optical depth bigger than a threshold. But first the UM is coarse
...
thresh_tau_driver = 5; %g/m2  -- Need to check the threshold for GOES -
% the min Tau available in file?
...
%        y_axis_vals = 'GOES LWP';
        ...
        ylabelstr = ['0.25^{o} cloud fraction for Tau.GT.' num2str(thresh_tau_driver)];
...
%        N=10; M=10;
     ...
     %box with an optical depth greater than a threshold. Total no. points =N*M
        %Make an array of ones and make the ones that we don't want to
        %count zero
        Ntau = zeros(size(goes_Tau));
        Ntau(goes_Tau>=thresh_tau_driver) = 1;
        %Now coarse grain (avearge) the Ntau array - this will now be our
        %cloud fraction
        cf_GOES_0pt25 = reduce_matrix_subsample_mean(Ntau,N,M);
...
for idat=1:99
    ...
    var_flag{idat}{1}=''; %Flag to say that we do not have this variable (e.g. Nd for old mphys - assume constand Nd)
        %Can specify multiple ones, e.g. var_flag{idat}{i}
end
...
idat=1;
%fileUM{idat} = 'xkqkf'; labs_UM(idat).l = '(xkqkf) Old-mphys'; flag{idat}='load_mat'; fileUM_rho{idat} = 'xkqkf_rho_.pp.nc'; pole_lat=70; pole_lon=278; idat=idat+1;
%%fileUM{idat} = 'xkqkf'; labs_UM(idat).l = '(xkqkf) Old-mphys'; var_flag{idat}{1}='Nd'; pole_lat=70; pole_lon=278; idat=idat+1;
...
for idat_UM=1:length(fileUM)
    ...
    for ivar=1:length(vars_UM)
        for iflag=1:length(var_flag{idat_UM})
           if strcmp(vars_UM{ivar},var_flag{idat_UM}{iflag}) == 1
              no_var=1;
              break
           else
               no_var=0;
...
%         vars_in.file_rho = ''; %filename_rho;
     ...
     [tau,time_out,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,it_driver,daynum_timeseries3_UM,modisyear_timeseries3_UM,gcm_time_UTC_UM,gcm_time_matlab_UM] = get_LWP_RWP_UM(vars_in);
...
%         dlon_GOES = meanNoNan(meanNoNan(d,1),1);
        ...
        tau_UM_n5 = reduce_matrix_subsample_mean(tau,N,M);
...
% is the same aas AMSRE)
    ...
    %resolution
     ...
     %Cloud fraction will be defined as fraction of points within each N*M
     %box with an tau greater than a threshold. Total no. points =N*M
        %Make an array of ones and make the ones that we don't want to
        %count zero
        Ntau = zeros(size(tau_UM_n5));
        Ntau(tau_UM_n5>thresh_tau_driver)=1;
        %Now coarse grain (avearge) the Ntau array - this will now be our
        %cloud fraction
        cf_UM_0pt25 = reduce_matrix_subsample_mean(Ntau,N,M);
