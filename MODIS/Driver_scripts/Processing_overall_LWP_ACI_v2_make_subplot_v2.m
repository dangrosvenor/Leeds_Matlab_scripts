...
for i=[1:length(labs_UM)] %idat_micro   %length(h)-2
...
%uistack(h(1).h,'top');
col{1}='b--'; col{2}='b';
for j=1:length(idat_micro) %cell array
    %Joint together the REMSS satellite values with a line.
    x_all=[]; y_all=[];
    for i=idat_micro{j}  %1:length(xdat)-2
    ...
    plot(x_all,y_all,col{j},'linewidth',3);
...
set(gca,'position',[pos(1) pos(2) pos(3) pos(4)*1.25]); 
