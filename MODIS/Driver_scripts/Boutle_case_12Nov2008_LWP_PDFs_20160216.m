...
logbin_norm_driver = 0; %0
i_plot_norm_driver=1; %1 %Whether to normalise
i_div_bin_widths_driver=1; %1  %whether to divide by the bin widths (also normalises)



%% Use iasc_desc to switch between day and night
...
Ybins_DRIVER = [-0.01 10.^[log10(20):0.15:log10(2500)]]; ichoose_Ybins=1;   %As used in 12th Nov paper until 7th Oct, 2016- prob still ok for daytime
%Ybins_DRIVER = [-0.01 10.^[log10(30):0.25:log10(2500)]]; ichoose_Ybins=1;   %WTryign larger bins to get Poisson error down (partiuculary for lower bins for night)
...
%Calculate some Poisson error stats relative error = sqrt(N)/N for 
%each bin of the PDFs
% *** Run without normalization and dividing by bin widths!

clear Poisson
for i=1:length(ydat_import)
   
    Poisson(i,:) = sqrt(ydat_import(i).y) ./ ydat_import(i).y;
