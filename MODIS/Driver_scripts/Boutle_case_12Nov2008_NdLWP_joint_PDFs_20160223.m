...
Xbins_DRIVER = [-0.01 10.^[log10(5):0.05:log10(300)]]; Xbins_driver = Xbins_DRIVER; %FOr LWP - will get overwritten if plotting
...
%thresh_LWP_DRIVER = 0; %g/m2 threshold to report GOES Nd
...
line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.4 0.4]; marker_styleUM(idat).m='d'; idat=idat+1;
fileUM{idat} = '/xlyd-y/xlydy_Nd_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-Ndvar-10';  flag{idat} = 'load_mat'; fileUM_Nd{idat} = '/xlyd-y/xlydy_Nd_.pp.nc.mat'; pole_lat=70; pole_lon=284;
line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0 0 0]; marker_styleUM(idat).m='v'; idat=idat+1;
...
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
     ...
     % Load the LWP in call cases as it is used for screening for LWP > thresh_LWP_DRIVER
...
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
     ...
     %Default values
     xlabelstr = 'LWP (g m^{-2})';
     ylabelstr = ['Nd for LWP.GT.' num2str(thresh_LWP_DRIVER) ' g m^{-3}, cm^{-3}'];
             

     switch x_var                      
         case 'W'
             %The updraft at the location of the max LWC is also calculated with the Nd in calc_LWP_multi_UM as of 23rd Feb, 2016.
             vars_in.var = 'W';
%             vars_in.var = 'wmax';             
             vars_in.file_lwp =  [dirUM fileUM{idat_UM}];
             vars_in.flag = flag{idat_UM};
             ...
             xlabelstr = 'Updraft (m s^{-1})';
             Xbins_DRIVER = [-1:0.01:1]; Xbins_driver = Xbins_DRIVER; %
     end
    
     [x_dat_UM,time_matlab,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,it_driver,daynum_timeseries3_UM,modisyear_timeseries3_UM,gcm_time_UTC_UM,gcm_time_matlab_UM] = get_LWP_RWP_UM(vars_in);
...
%            dlon_target = dlon_AMSRE;
        ...
        lwp_UM_n5 = reduce_matrix_subsample_mean(x_dat_UM,N,M);
