%LWP and Nd maps from GOES data.
...
LAT_val_DRIVER = [-25.4100  -14.5700]; LON_val_DRIVER =[-81.8550  -70.1550]; % 2 x FULL UM domain for 12th Nov
...
%      [lwp,time_matlab,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,it_driver,daynum_timeseries3_UM,modisyear_timeseries3_UM,gcm_time_UTC_UM,gcm_time_matlab_UM] = get_LWP_RWP_UM(vars_in);
    ...
    for it_driver=1:131 %79:79 %1:length(time_GOES_save)  %4:nt_driver
        ...
        switch goes_var
            case 'Nd'
                ...
                dat_modis = squeeze(goes_Nd_multi{it_driver}(:,:)); %already in g/m2
            ...
            case 'LWP'
                ...
                dat_modis = squeeze(goes_LWP_multi{it_driver}(:,:)); %already in g/m2
        ...
        titlenam_driver=[datestr(time,'YYYYmmDDHHMM') '_' titlenam_driver];
