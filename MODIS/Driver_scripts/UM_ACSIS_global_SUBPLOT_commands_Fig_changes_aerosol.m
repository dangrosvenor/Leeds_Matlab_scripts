...
fig_3panel_savename = 'changes_aerosol';
icoarse_grain=0;
...
%% ----  AOD (total) ---------------------------
    ...
    %um_data = 
    dat_load = load([savedir_date 'Percentage_change_in_total_AOD_(PD_minus_PI).mat']);
...
for i=1:6
    ...
    %3rd columns)
    Hc = find_peer_colorbars_of_an_axes(a);    
    pos = get(Hc,'position');    
    pos(2) = pos(2) + dY_pos_ALL;
    set(Hc,'position',pos);
end
