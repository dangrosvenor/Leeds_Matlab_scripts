% Plot seasonal cycles of MODIS Nd separately for different wavelengths
% using 2007 mock L3 data.
% Saved this data using save_vars_mat_run
% Runs monthly_means_from_plot_global


 % *** Remember that the data in mockL3 is backwards in lat dimension !! ***
%Might need to calculate dSZA using:-
...
modis_data_plots ={'Number of droplets differences between datasets'...
    };
...
set_screening = {'NP + CF_L3'};
set_screening = {'NP + CF_L3 + max_SZA + std_SZA'};
set_screening = {'NP + CF_L3 + dSZA'};
...
screen_edits = {'thresh_SZA=[0 90];'};
screen_edits = {'thresh_SZA=[0 90]; thresh_maxSZA=[-1 81.4]; thresh_stdSZA=[-1 1e9];'};
screen_edits = {'thresh_SZA=[0 90]; thresh_CF=[-0.05 1.00001];'};
%screen_edits = {'thresh_SZA=[0 90]; thresh_CF=[-0.05 1.00001]; thresh_maxSZA=[-1 81.4]; thresh_stdSZA=[-1 1e9];'};
screen_edits = {'thresh_SZA=[0 90]; thresh_CF=[-0.05 1.00001];'};
screen_edits = {'thresh_SZA=[0 90]; thresh_maxSZA=[-1 81.4]; thresh_stdSZA=[-1 0.5]; thresh_CF=[-0.05 1.00001];'};
screen_edits = {'thresh_SZA=[0 90]; thresh_CF=[0.8 1.00001]; thresh_dSZA=[-0.0001 1];'};
%screen_edits = {'thresh_SZA=[0 90]; thresh_CF=[0.8 1.00001];'};
...
ifilter_ndays=0;
%inew_cticks=0;  %this is overridden in monthly_means_from_plot_global
%anyway...
            ...
            for iloop_MOD=1:length(set_MOD)
                ...
                for iloop_abs=1:length(set_real_abs)
                    ...
                    for iloop_years=1:length(set_years)
                        ...
                        LAT_vals = [-60 -40;]; LON_vals = [-180 180;]; %Southern Ocean
                        ...
                        season_zonal_driver{icount_DRIVER} = season_mean_vs_LAT_2D;
                        season_Ndatap_zonal_driver{icount_DRIVER} = season_Ndatap_vs_LAT_2D;
...
% --- How to plot the 2D lat vs month mean bias
%MLAT_edges=[MLAT-0.5 MLAT(end)+0.5];
%figure
%dpcolor([1:13],flipdim(MLAT_edges,2),season_mean_vs_LAT_2D)
%caxis([-20 20]); colorbar; titlenamw=textwrap({[thresh_str ' for L3, SZA<' num2str(maxALL(G14.Solar_Zenith_Maximum.timeseries3)) ' for mockL3']},70); title(titlenamw)
