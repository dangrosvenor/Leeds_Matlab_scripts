...
% Creates PDFs LWP for model
       ...
       iplot_AMSRE = 1; %Whether to do the AMSRE PDF
       iplot_model = 0; %Whether to do the model PDF
       
% ------- Select filename to save to and what type of plot, etc. ---------       
%       var_str_file = 'TLWP_PDFs_CAPT_ocean_only';
%       var_str_file = 'LWP_PDFs_CAPT_ocean_only';  
       var_str_file = 'TLWP_PDFs_CAPT_ocean_only_Sc_deck';
       var_str_file = 'LWP_PDFs_CAPT_ocean_only_Sc_deck';        
       var_str_file = 'LWP_PDFs_AMSRE_clear-sky_bias_tests'; %Whole region (i.e. not restricted to Sc deck), but with res degraded to degrees         
% ----------------------------------------------------------------  

%Not needed for AMSRE
switch var_str_file
    case {'TLWP_PDFs_CAPT_ocean_only','TLWP_PDFs_CAPT_ocean_only_Sc_deck'}
        LWP_or_TLWP = 'TLWP'
    case {'LWP_PDFs_CAPT_ocean_only','LWP_PDFs_CAPT_ocean_only_Sc_deck','LWP_PDFs_CAPT_ocean_only_2deg'}
        LWP_or_TLWP = 'LWP'
...
%         modis_data_plots={'LWP AMSRE time3'}; fraction_plotcase='Fraction of days remaining after screening for AMSRE bias comp';
        ...
        switch var_str_file
            case {'TLWP_PDFs_CAPT_ocean_only','LWP_PDFs_CAPT_ocean_only','LWP_PDFs_CAPT_ocean_only_2deg'}
            ...
            case {'LWP_PDFs_CAPT_ocean_only_Sc_deck','TLWP_PDFs_CAPT_ocean_only_Sc_deck'}
...
screen_edits = {'thresh_CF=[-0.01 0.01]','thresh_CF=[-0.01 0.03]','thresh_CF=[-0.01 0.05]','thresh_CF=[-0.01 0.07]','thresh_CF=[-0.01 0.09]','thresh_CF=[-0.01 0.2]','thresh_CF=[-0.01 0.4]','thresh_CF=[-0.01 0.5]','thresh_CF=[-0.01 0.6]','thresh_CF=[-0.01 0.8]','thresh_CF=[-0.01 1.01]'};
...
%     };  %ranges of CF (applies to model too when choosing re CF range)
        ...
        amsre_daynight_labels = {'daytime'}; %Just daytime since need MODIS CF      
...
%    nYpdf=8;    
    ...
    ioverride_Ybins=1;
    Ybins = [-60:2:100]; ichoose_Ybins=1;
%    Ybins = [-60:1.5:100]; ichoose_Ybins=1;    
...
%    ------- Model LWP PDF ---------------------------------                       
                            ...
                            %x_axis_vals and y_axis_vals are what are used
                            %to select what to plot in pdf2D_plot_commands
...
%                            x_axis_vals = 'LWP GCM grid-box mean'; %dummy data
                            ...
                            if length(strfind('CAM',gcm_str))>0 %Only for CAM models
                                ireduce_res_lwp=1; %Tell it to reduce the res (to 2x2 deg to match AM3 models)
                                M_lwp=2; N_lwp=2; %since are soing from 1x1 to 2x2 deg.
                            end
...
% --------------------------- now do the AMSRE LWP      --------------------
                            ...
                            %Overrideing choice from set_screening here
                            %(may no want to do this...)
%                            screen_type = 'gcm_screening';
...
%                            x_axis_vals = 'Grid-box mean AMSRE TLWP';   %dummy data
                            ...
                            gcm_str = 'AMSRE_time3';  %This sets it to use the time3 data (matched to MODIS)
                                %Time specification will be different to
                                %using year_amsre etc.
...
%                            days_required_for_mean = [1:366]; time_mean_str = 'ANNUAL';
                            ...
                            ireduce_res_lwp=0; %Tell it to reduce the res (to 2x2 deg to match AM3 models)
                            M_lwp=2; N_lwp=2; %since are soing from 1x1 to 2x2 deg.
...
%                            end
                            ...
                            per_str = num2str(thresh_CF(2));
                            per_str = strrep(per_str,'.','pt');
                            gcm_str = [gcm_str '_' per_str];
...
clear ioverride_Ybins
catch amsre_pdf_error
    clear ioverride_Ybins
    rethrow(amsre_pdf_error)
end
