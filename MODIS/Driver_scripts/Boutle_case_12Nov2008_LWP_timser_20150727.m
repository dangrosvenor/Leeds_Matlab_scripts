...
day_or_night = 'all'; %Used for the calculation of LWP_overall_mean
...
%FULL UM domain for 12th Nov
  LAT_val_DRIVER = [-22.70 -17.28]; LON_val_DRIVER =[-78.93 -73.08]; 
% N.B. - if changing this, may need to re-do the obs files since these use
% .mat files that were based on the top half of the UM domain. If done
% before, will need to change the filename
...
%fileUM{idat} = '/xlyd-x/xlydx_LWP_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-Nd-fixed-act';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xlyd-x/xlydx_rho_.pp.nc'; pole_lat=70; pole_lon=284;
%line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.4 0.4]; marker_styleUM(idat).m='d'; idat=idat+1;
%fileUM{idat} = '/xlyd-y/xlydy_LWP_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-Ndvar-10-new';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xlyd-y/xlydy_rho_.pp.nc'; pole_lat=70; pole_lon=284;
...
%line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.8 0]; marker_styleUM(idat).m='^'; idat=idat+1;
%fileUM{idat} = '/xlyd-q/xlydq_LWP_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-fixed-act-NoRain';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xlyd-q/xlydq_rho_.pp.nc'; pole_lat=70; pole_lon=284;
%line_patternUM(idat).p= '--';  line_colourUM(idat).c=[1 0.0 0.0]; marker_styleUM(idat).m='<'; idat=idat+1;
%fileUM{idat} = '/xlyd-r/xlydr_LWP_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-fixed-act-NoRain-Less-Mixing';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xlyd-r/xlydr_rho_.pp.nc'; pole_lat=70; pole_lon=284;
%line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.8 0.0]; marker_styleUM(idat).m='<'; idat=idat+1;
%fileUM{idat} = '/xlyd-s/xlyds_LWP_.pp.nc.mat'; labs_UM(idat).l = 'CASIM-Ndvar-10-NoRain-Less-Mixing';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xlyd-s/xlyds_rho_.pp.nc'; pole_lat=70; pole_lon=284;
...
%fileUM{idat} = '/xmmz-l/xmmzl_LWP_.pp.nc.mat'; labs_UM(idat).l ='CASIM-Ndvar-RHcrit0.999';  flag{idat} = 'load_mat'; fileUM_rho{idat} = '/xmmz-v/xmmzv_rho_.pp.nc'; pole_lat=70; pole_lon=284;
 %   line_patternUM(idat).p= '-';  line_colourUM(idat).c=[0 1 0]; marker_styleUM(idat).m='v'; idat=idat+1;
...
%     rwp=permute(rwp,[3 1 2]); 
    ...
    ydat_import(idat_driver).y = meanNoNan(tlwp(:,:),2);
    
    it_overall = find(time_matlab >= time(1) & time_matlab <= time(end) );
    
    lwp_temp = meanNoNan(tlwp(it_overall,:,:),1);
...
%Saving of LWP_overall_mean - latest file is :-
        % '/home/disk/eos8/d.grosvenor/UM/12Nov2008_Boutle/LWP_overall_20160707T101357.mat'
%save_LWP_overall_file = [dirUM 'LWP_overall_' datestr(now,30) '.mat'];
%save(save_LWP_overall_file,'LWP_overall_mean','time','time_matlab','it_overall','labs');
