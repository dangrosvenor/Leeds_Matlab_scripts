...
%Runs with enhanced aerosol (as of July 2016) :-
...
%     
         ...
         vars_in.var = 'Nd';
         vars_in.flag = flag{idat_UM};
         vars_in.file_lwp =  [dirUM remove_character(fileUM{idat_UM},'LWP','Nd')];   %Keep as file_lwp for loading .mat files
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
     ...
     [Nd,time_matlab,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,it_driver,daynum_timeseries3_UM,modisyear_timeseries3_UM,gcm_time_UTC_UM,gcm_time_matlab_UM] = get_LWP_RWP_UM(vars_in);
         ...
         vars_in.var = 'LWP';
         vars_in.flag = flag{idat_UM};
         vars_in.file_lwp =  [dirUM remove_character(fileUM{idat_UM},'Nd','LWP')];   %Keep as file_lwp for loading .mat files
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
         vars_in.pole_lat = pole_lat;
         vars_in.pole_lon = pole_lon;
         vars_in.time_in = []; %set to this for all times
