...
fileUM{idat} = '/xlyd-y/xlydy_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CASIM-Ndvar-10-new';  flag{idat} = ''; fileUM_rho{idat} = '/xlyd-y/xlydy_rho_.pp.nc'; pole_lat=70; pole_lon=284;
line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0 0 0]; marker_styleUM(idat).m='v'; idat=idat+1;
fileUM{idat} = '/xlyd-p/xlydp_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CASIM-Ndvar-10-NoRain';  flag{idat} = ''; fileUM_rho{idat} = '/xlyd-p/xlydp_rho_.pp.nc'; pole_lat=70; pole_lon=284;
line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.8 0]; marker_styleUM(idat).m='^'; idat=idat+1;
fileUM{idat} = '/xlyd-z/xlydz_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CASIM-Ndvar-10-SWNd';  flag{idat} = ''; fileUM_rho{idat} = '/xlyd-z/xlydz_rho_.pp.nc'; pole_lat=70; pole_lon=284;
line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.6 0.6 0]; marker_styleUM(idat).m='>'; idat=idat+1;




    case 'Cloud_scheme_tests'
       
        idat=1;

% - Tests to examine the effect of not using the CF in mphys for cloud
% scheme
fileUM{idat} = '/u-ab793/cld_scheme/atomic_output/u-ab793_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'Cld scheme';  flag{idat} = ''; pole_lat=70; pole_lon=284; 
    line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.4 0.4]; marker_styleUM(idat).m='d'; idat=idat+1;
% High and low aerosol cases.
fileUM{idat} = '/u-ab793/mphys_CF_1pt0/atomic_output/u-ab793_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CF=1 for mphys only';  flag{idat} = ''; pole_lat=70; pole_lon=284;
        line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0 0 0]; marker_styleUM(idat).m='v'; idat=idat+1;
fileUM{idat} = '/u-ab793/rad_CF_1/atomic_output/u-ab793_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CF=1 for rad only';  flag{idat} = ''; pole_lat=70; pole_lon=284;
        line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.8 0.4 0]; marker_styleUM(idat).m='^'; idat=idat+1;
fileUM{idat} = '/u-ab793/div_cfrac_tidy_only/atomic_output/u-ab793_SW_up_TOA_.pp.nc'; labs_UM(idat).l = 'CF=1 for tidy only';  flag{idat} = ''; pole_lat=70; pole_lon=284; 
        line_patternUM(idat).p= '--';  line_colourUM(idat).c=[0.4 0.8 0]; marker_styleUM(idat).m='^'; idat=idat+1;
...
%     
         ...
         vars_in.flag = flag{idat_UM};
         vars_in.file_lwp =  [dirUM fileUM{idat_UM}];   %Keep as file_lwp for loading .mat files
%         vars_in.file_rho = [dirUM fileUM_rho{idat_UM}]; %filename_rho;
         vars_in.pole_lat = pole_lat;
         vars_in.pole_lon = pole_lon;
         vars_in.time_in = []; %set to this for all times
    
     [dat_UM,time_matlab,gcm_Plat2D_UM,gcm_Plon2D_UM,gcm_Plat2D_edges_UM,gcm_Plon2D_edges_UM,it_driver,daynum_timeseries3_UM,modisyear_timeseries3_UM,gcm_time_UTC_UM,gcm_time_matlab_UM] = get_LWP_RWP_UM(vars_in);
    ...
    dat_UM=permute(dat_UM,[2 3 1]);
    a=NaN*ones(size(dat_UM));
    a(iregion_lin)=0;
    dat_UM=dat_UM+a;
    dat_UM=permute(dat_UM,[3 1 2]);
...
%    tlwp = lwp+rwp;
    ...
    ydat_import(idat_driver).y = meanNoNan(dat_UM(:,:),2);
