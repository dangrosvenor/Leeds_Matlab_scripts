function [reff,H,k,Q,cw,tau]=MODIS_re_func_W_and_Nd(W,N,CTT,CTP,fad,k)
%function [reff,H,k,Q,cw,tau]=MODIS_re_func_W_and_Nd(W,N,CTT,CTP,fad)
%  
% Returns reff in metres and H in metres 
% as a funciton of W in kg/m2 and N in m^-3 (=cm^-3 * 1e6)
% Also returns tau (unitless)
% reff - effective radius in metres
...
reff = ( 3*sqrt(2.*fad*cw.*W) ./ (4.*pi.*k.*rhoL.*N) ).^(1/3);
...
H=sqrt(2*W./(fad*cw));  %cloud depth (m)
