function [seaice_time3,seaice_max_time3] = seaice_match_times_FUNC(seaice_struct,Cloud_Fraction_Liquid,modisyear_timeseries3,daynum_timeseries3,ignore_seaice_warn)

if ~exist('ignore_seaice_warn')
    ignore_seaice_warn=0;
end
...
clear daynum_seaice
for it=1:length(seaice_struct.seaice_array_1deg_datenum)
    daynum_seaice(it) = day_of_year_from_date_func(seaice_struct.seaice_array_1deg_datenum(it));
end
[year_seaice,M,D] = datevec(seaice_struct.seaice_array_1deg_datenum);
...
for it=1:length(modisyear_timeseries3)
    ...
    if length(i)>0
       seaice_time3(:,:,it) = seaice_struct.seaice_array_1deg(:,:,i);
       seaice_max_time3(:,:,it) = seaice_struct.seaice_array_1deg_max(:,:,i);
        ...
        if strcmp(ain,'stop')==1 & length(iex)==0 & ignore_seaice_warn==0
