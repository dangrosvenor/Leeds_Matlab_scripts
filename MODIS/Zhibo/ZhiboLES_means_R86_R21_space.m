...
Xbins_DRIVER = [0:0.05:0.7]; ichoose_Xbins=1;
Ybins_DRIVER = [0:0.025:0.25]; ichoose_Ybins=1;

%% Define the data somewhere
iview=7; %iview=7 is nadir view
...
switch H_case
    case '086'
        switch LES_case
            case 'ATEX'
                X_driver = SZA_20_ATEX.R86_1km_3D(iview,:,:); %the data
                Y_driver = SZA_20_ATEX.R21_1km_3D(iview,:,:); %the data
                Z_driver = SZA_20_ATEX.H86_1km_3D(iview,:,:).^2;
                title_DRIVER = 'ATEX H_{086}^2';
        end
     case '21'
         switch LES_case
             case 'ATEX'
                 X_driver = SZA_20_ATEX.R86_1km_3D(iview,:,:); %the data
                 Y_driver = SZA_20_ATEX.R21_1km_3D(iview,:,:); %the data
                 Z_driver = SZA_20_ATEX.H21_1km_3D(iview,:,:).^2;
                 title_DRIVER = 'ATEX H_{21}^2';
         end
