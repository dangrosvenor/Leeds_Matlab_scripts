...
ijust_final_plot = 1;  %Flag to tell it to just do the final plot without re-calculating/re-loading, etc.
...
%        Ybins = [-0.01 10.^[log10(30):0.1:log10(2500)]]; ichoose_Ybins=1;
        Ybins = [220:5:303]; ichoose_Ybins=1;
...
%        gcm_str = gcm_str_last_loaded;        
 ...
 % --------- Override flags for 2D PDF --------
        ...
        labs_import(idat_driver).l = remove_problem_chars(set_year{iloop_year}{1}(1:5));
        xlab_import = xlab;
        ylab_import = ylab;
%        ioverride_savePDF=1;
%        save_1D_pdfs;
...
titlenam = 'CTT PDFs multiple years';
xlab='CTT (K)';
ylab = ylab_import;
xlims=0;
xlimits=[0 100];
