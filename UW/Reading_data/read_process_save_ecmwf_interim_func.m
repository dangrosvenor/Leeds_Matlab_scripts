function [ecT_daily_Man,time_daily,ecT_mon_Man,ecLat_Man,ecLon_Man,levs_Man_L6,ecLTS_mon_Man] = read_process_save_ecmwf_interim_func(nc_file,var_str_ec)

%Read in ECMWF LTS, etc from Dargan's data

% size(ecT_Man) = [ 1460           6         121         240 ] 
%  -- [ 4*365=time 6=levels 121=lat 240=lon]
...
%        direc_Man = '/home/disk/eos5/d.grosvenor/ERA_Interim/ManuelZ/';
        nc_ecT_Man = netcdf([nc_file],'nowrite');
        ...
        ecT_Man = nc_ecT_Man{var_str_ec}(:,:,:,:);  %K

        %calculate the daily averages (average of 0,6,12 and 8 hours)
        ...
        %old way of calculating LTS using the monthly mean T
        %        ecLTS_mon_Man = squeeze(potemp(ecT_mon_Man(:,:,3,:,:),700e2) - potemp(ecT_mon_Man(:,:,6,:,:),1000e2));
        % new way using the daily T values to calc LTS and then doing
        % monthly average
        % calc LTS using the 700 and 1000mb levels
        ecLTS_Man = squeeze(potemp(ecT_Man(:,3,:,:),700e2) - potemp(ecT_Man(:,6,:,:),1000e2));
        [ecLTS_daily_Man,ecLTS_mon_Man,temp] = ecmwf_averages(ecLTS_Man,ecTime_Man);
