fprintf(1,'\n*** WARNING - don''t use this one, use read_am3T\nRemove this stop if intentional***\n');
return    
...
gcm_lat = nc_grid{'grid_latt'}(:);
gcm_lon = nc_grid{'grid_lont'}(:);

gcm_lat2 = nc_grid{'grid_lat'}(:);
gcm_lon2 = nc_grid{'grid_lon'}(:);
...
gcm_ps = nc_inst{'ps'}(gcm_idays,:,:); %surface pressure (time,lat,lon)
...
gcm_rho = density(gcm_pfull,gcm_temp);
gcm_tsurf = nc_inst{'t_surf'}(gcm_idays,:,:); %surface temperature in K
gcm_zsurf = nc_inst{'zsurf'}(:,:); %surface temperature in K

gcm_landmask = nc_inst{'land_mask'}(:); %radiative surface temperature (time,lat,lon), K
...
%[h_full,h_half]=gcm_calc_height(gcm_pfull,gcm_temp,gcm_phalf);
...
% -----------------------------------
gcm_cf = nc_inst{'cld_amt'}(gcm_idays,:,:,:); %cloud fraction between 0 and 1
...
gcm_drop_read=nc_inst{'liq_drp'}(gcm_idays,:,:,:) ./ gcm_cf .*gcm_rho; %multiply by density to convert to per m3 (from per kg)
gcm_drop_read2=nc_inst{'liq_drp'}(gcm_idays,:,:,:).*gcm_rho; % #/kg
%gcm_drop = nc_inst{'strat_droplet_number'}(gcm_idays,:,:,:) ./ gcm_cf;
gcm_liq=nc_inst{'liq_wat'}(gcm_idays,:,:,:) ./ gcm_cf; 
gcm_liq_av=nc_inst{'liq_wat'}(gcm_idays,:,:,:);
gcm_lwc=gcm_liq .* gcm_rho; %convert to kg/m3
gcm_lwc_av = gcm_liq_av.* gcm_rho; %convert to kg/m3;

gcm_iwc_av=nc_inst{'ice_wat'}(gcm_idays,:,:,:);


gcm_lwp_all_clouds=nc_inst{'LWP_all_clouds'}(gcm_idays,:,:); %is this cloud averaged?
gcm_lwp=nc_inst{'LWP'}(gcm_idays,:,:); %how to deal with CF?
