...
file_name_h5 = 'MOD06_L2.A2020366.2315.061.2021268053837.hdf';
...
[tau21_pcl,dimsizes_tau_pcl]=get_hdf_data_dan('Cloud_Optical_Thickness_PCL',SD_id,INFO); %int16 (>0)
[tau16_pcl,dimsizes_tau_pcl]=get_hdf_data_dan('Cloud_Optical_Thickness_16_PCL',SD_id,INFO); %int16 (>0)
[tau37_pcl,dimsizes_tau_pcl]=get_hdf_data_dan('Cloud_Optical_Thickness_37_PCL',SD_id,INFO); %int16 (>0)
...
[re21_pcl,dimsizes_re_pcl]=get_hdf_data_dan('Cloud_Effective_Radius_PCL',SD_id,INFO); %int16, (401 min) sf=0.01 (microns)
[re16_pcl,dimsizes_re_pcl]=get_hdf_data_dan('Cloud_Effective_Radius_16_PCL',SD_id,INFO); %int16, (401 min) sf=0.01 (microns)
[re37_pcl,dimsizes_re_pcl]=get_hdf_data_dan('Cloud_Effective_Radius_37_PCL',SD_id,INFO); %int16, (401 min) sf=0.01 (microns)
...
[cloud_phase,dimsizes_cloud_phase]=get_hdf_data_dan('Cloud_Phase_Optical_Properties',SD_id,INFO); %int16 (>0)
  % 0 = cloud mask unavailable, missing data,etc. (no phase result), 1= no phase result due to clear sky, etc.; 
...
cloud_phase_flag = squeeze(qapq_1km(8,:,:)); %0: SWIR algorithm not run, 1: CLEAR, 2: WATER, 3: ICE, 4: MIXED PHASE or UNDETERMINED
cloud_mask_status_flag = squeeze(mask_1km(1,:,:));
cloud_mask_confidence_flag = squeeze(mask_1km(2,:,:));
...
%-
    ...
    re21_un_pcl(ind)=NaN; 
    re37_un_pcl(ind)=NaN; 
    tau16_un_pcl(ind)=NaN;   %
    tau21_un_pcl(ind)=NaN; 
    tau37_un_pcl(ind)=NaN; 
...
%% Calculate some things - Nd, LWP, lat/lon, etc.
