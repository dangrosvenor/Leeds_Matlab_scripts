...
[lat_10km,dimsizes_lat,sampling_along_10km,sampling_across_10km,success]=get_hdf_data_dan('Latitude_10km',SD_id,INFO);

%if success==0 %if it failed to read Latitude_10km
        
%end

[lon_10km,dimsizes_lon]=get_hdf_data_dan('Longitude_10km',SD_id,INFO);
...
[tau,dimsizes_tau,tmp,tmp,success_tau]=get_hdf_data_dan('Cloud_Optical_Thickness',SD_id,INFO); %int16 (>0)
if success_tau==0
    return
end

[qa1,dimsizes_QA_1km]=get_hdf_data_dan('Cloud_Quality_Assurance',SD_id,INFO,1); %int8 (-128 min) unsigned
...
[mask5,dimsizes_mask5]=get_hdf_data_dan('Cloud_Mask',SD_id,INFO,1); %int8 unsigned
...
[re_diff,dimsizes_re]=get_hdf_data_dan('Cloud_Effective_Radius_Difference',SD_id,INFO); %int16, (401 min) sf=0.01 (microns)
...
[solar_zenith,dimsizes_solar_zenith]=get_hdf_data_dan('Solar_Zenith_10km',SD_id,INFO); %int16, min=-32767
%[solar_azimuth,dimsizes_sensor_zenith]=get_hdf_data_dan('Solar_Azimuth_10km',SD_id,INFO); %int16
[sensor_zenith,dimsizes_sensor_zenith]=get_hdf_data_dan('Viewing_Zenith_10km',SD_id,INFO); %int16
[relative_azimuth,dimsizes_sensor_zenith]=get_hdf_data_dan('Relative_Azimuth_10km',SD_id,INFO); %int16
...
%% Stuff that probably will never be needed...
...
%% Read in and process the cloud mask and flags - only 5km available for Joint-L2 I think.
...
%mask_1km set to be the same as mask_5km in filtering_data_joint_L2

%NOTE - for Joint L2 the qa flag is the same as the 1 km qa flag
%qapp_5km is a continuation of the 1km QA as documented in the MODIS QA
%plan document. Starts at the 3rd byte ("Primary Cloud Retrieval Phase
%Flag").
%[mask_1km] = maskread_1km_Dan(mask1);

%% Decide which pixels to do Nd, reff, tau retrievals for RE water path confidence, etc.

%    ind=where(mask_1km(1,*,*) gt 0 or qapq_5km(6,*,*) lt 3)
%    ;  lwp(ind)=0.00

%changed this so that it also screens for mask_1km(1,:,:) - Cloud Mask Status Flag  
% 0 = Undetermined, 1 = Determined. Although prob not any undetermined cloud points with good confidence 
%  qapq_5km(6) : WATER PATH CONFIDENCE - this is qapq_5km(7) in Matlab
...
%  **** N.B. - need to add one to the following indices for Matlab 1-based indices
%  ************************************************************************
%===========================================================================
...
%
%  qapq_5km(0) : OPTICAL THICKNESS RETRIEVAL 
%                    0: not useful% 1 useful
%  qapq_5km(1) : OPTICAL THICKNESS CONFIDENCE 
%                    0: bad% 1 marginal% 2 good% 3 very good
%  qapq_5km(2) : OPTICAL THICKNESS OUT OF BOUNDS 
%                    0: within bounds (tau<150)
%                    1: (100<tau<150)
%                    2: (tau>150)
%                    3: surface reflectance too large
%  qapq_5km(3) : EFFECTIVE RADIUS GENERAL QA
%		     0: not useful
% 		     1: useful
%  qapq_5km(4) : EFFECTIVE RADIUS CONFIDENCE 
%                    0: bad% 1 marginal% 2 good% 3 very good			
%  qapq_5km(5) : WATER PATH GENERAL QA
%		     0: not useful
% 		     1: useful
%  qapq_5km(6) : WATER PATH CONFIDENCE 
%                    0: bad% 1 marginal% 2 good% 3 very good
%  qapq_5km(7) : CLOUD PHASE DETERMINATION
...
%  qapq_5km(8) : 1621 cloud retrieval outcome
