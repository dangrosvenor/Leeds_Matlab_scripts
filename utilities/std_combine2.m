...
%Combine the stds (and means) from several different samples (when
%combinging to find the overall mean across all the data)
% stds, means and Ns are vectors of length n (size [1 n]) of the std devs, means and sample sizes
...
% (under section "Combining standard devs"
...
Ntot_rep = repmat(Ntot,[1 size(stds,2)]);
%The following two are actualy identical, but the second matches the Wiki
%page - https://en.wikipedia.org/w/index.php?title=Standard_deviation&oldid=628884034#Combining_standard_deviations
%Part about M non-overlapping populations.
%std_out = sqrt(  sum(Ns.*(stds.^2 + means.^2 - me_rep.^2) ,2) ./ Ntot  );
%std_out = sqrt(  sum(Ns.*(stds.^2 + means.^2),2)./Ntot - me_out.^2    );

%The following is the new formula on Wiki - this exactly matches what you
%get if you use the std of all of the combined data. The difference with
%the above is that it does the N-1 bias correction.
%https://en.wikipedia.org/wiki/Pooled_variance - part under "Effect on
%Precision).
std_out = sqrt(  ( sum( (Ns-1).*stds.^2 + Ns.*means.^2 , 2  )   - Ntot.*me_out.^2 )  ./ (Ntot-1) );
...
% s1 (using std_combine) = 27.86, stdA (true answer) = 27.83. But probably close enough!
run_test=0;
if run_test==1
    ...
    switch test_case
        ...
        case 2
            ...
            %This shoudl match the overall std of std_test0
            %Overall mean and std based on whole array - this is what we
            %should end up with at the end (for the seasonal average).
            [me0_test,N0_test,std_test0] = meanNoNan(d3(:),1);
            
            rerror0 = std_test0 ./ std_combined0
            
            %Generally get rerror values of around 0.985
