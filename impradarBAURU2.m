%quicker version of cloud height stats program

clear x a2 map c rhs time date dir

dire='c:/documents and settings/user/my documents/hibiscus/baurufield/radar/echotops/01-12.03/';
d=dir(dire);
[arad b]=size(d);
...
%         end
    ...
    %for j=1:478
              ...
              for k=1:size(hvals,2);
                  [rh rx]=find(im==hvals(k)); %find points with value hvals(k)
                  aa=size(rh);
                  if aa>0 
                    %for ih=1:aa;
                            [dmin(k) jd]=min(sqrt( (c2-rx).^2  + (rh-c1).^2 ));
